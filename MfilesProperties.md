...
**Clarifying note (addendum to Decision 7), 2026-08-19 — automatic-transition grammar authoring reaffirmed against an external "skeleton-only" proposal; that document's own changelog corrected on the record.** `mermaid-workflow-designer-prd.md`, an external document the operator brought in for review (not authored as part of this project's own documentation lineage), proposes an architecture where Mermaid carries only States, Transitions, and a boolean gateway flag — all `if()`/`after()`/`role()`/`script()` condition and trigger logic deferred entirely to a human working in M-Files Admin after the fact. Taken at face value, this would mean undoing the automatic-transition grammar-authoring feature (`after(Nd)`, `if(Property=Value)`, `script(Name)`, `+priority(N)`) this project built, live-verified with real `page.mouse`-driven Playwright runs, and committed against the addendum immediately above (`src/utils/transitionGrammar.js`, `commit 2599827`, 2026-08-16; see `progress.md`'s "Automatic-transition grammar authoring built" entry, same date, for the verification detail).

**Reviewed and confirmed: grammar authoring stays exactly as built. The external proposal is not adopted.** The reasoning is worth stating plainly, not just the outcome, because it's the same reasoning that has held for every M-Files Flow feature built since the Decision 7 addendum above, not a new argument invented for this document specifically: **legibility and translatability were never competing goals here, and the reason they aren't is structural, not coincidental.** The diamond badge (Decision 5), the hub badge, and the rendered condition label on a genuinely parsed `after()`/`if()`/`script()` edge are all, without exception, a **mechanical, read-only consequence of real transition data already sitting in the store** — never a second, independently-authored layer sitting alongside it that could drift out of sync with what the Translator will actually emit. That is precisely why drag-to-connect, the hub badge, and grammar authoring all integrated cleanly with each other and with the existing diamond mechanism on first build, with zero conflicts between them across the entire 2026-08-16 session: every one of them reads or writes the *same* underlying transition data through the *same* store, so there was never a second source of truth for any of them to disagree with. An external skeleton-only model that pushes condition logic out of the diagram entirely would not make the diagram more legible under this project's own convention — it would remove the one thing (§3.5's own stated premise, above Decision 5) that makes the diagram worth reading over the raw M-Files configuration in the first place: that the labels are the real mechanism, not a caption describing it from a distance.

**The external document's own §11 changelog is factually wrong about this project's history, and that discrepancy is recorded here rather than left to stand.** §11 states: *"No embedded M-Files field syntax in labels or node text (`if()`, `role()`, `+esign`, etc.)... This was tried and explicitly reversed."* No such reversal occurred. The grammar was proposed (§3.5's table, this document, predates the external PRD), built, verified live against real Playwright interaction, and shipped in a real commit — never attempted-then-rolled-back. Handled the same way this document has handled every other externally-sourced claim checked against this project's real code and commit history (e.g. the Qwen structuring audit's G1-G4 gaps, verified against vendor citations rather than accepted on description alone): checked against the actual repository state first, found incorrect, corrected on the record rather than silently ignored — so a future reader who encounters `mermaid-workflow-designer-prd.md` on its own isn't misled by a changelog entry describing a reversal that never happened in this project.
