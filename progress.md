...
- Collaboration features (deferred) — **SUPERSEDED 2026-08-21:** now a locked v1 feature for the separate React Flow Pro workflow designer (operator decision — explicit selling point against Cacoo). See this file's "React Flow Pro workflow designer" entry further down.
...
- libavoid obstacle-avoid routing (deferred pending feasibility) — **SUPERSEDED 2026-08-21:** feasibility confirmed (no special bundler plugin needed, Vite `?url` import handles the WASM asset); now a locked v1 edge type. See this file's "React Flow Pro workflow designer" entry further down.
- Server-side image export (deferred; no backend infra in place) — **SUPERSEDED 2026-08-21:** now a locked v1 feature for the separate React Flow Pro workflow designer (operator decision). See this file's "React Flow Pro workflow designer" entry further down.
...
## Grammar authoring reaffirmed against an external "skeleton-only" proposal; that document's changelog corrected on the record (2026-08-19)

The operator brought in an external document, `mermaid-workflow-designer-prd.md`, for review — not part of this project's own documentation lineage. It proposes deferring all `if()`/`after()`/`role()`/`script()` condition and trigger logic out of Mermaid entirely, to be configured afterward by a human in M-Files Admin — which, taken at face value, would mean undoing the automatic-transition grammar-authoring feature built and shipped 2026-08-16 (`src/utils/transitionGrammar.js`, `commit 2599827`, this file's "Automatic-transition grammar authoring built" entry above).

**Reviewed and confirmed: grammar authoring stays exactly as built, not adopted.** Full reasoning recorded in `MfilesProperties.md`'s new 2026-08-19 addendum (filed directly after the existing Decision 7 addendum, before Decision 8) rather than duplicated here — in short: legibility (the diamond/hub badges) and translatability were never competing goals in this project's design, because every visual signal on the canvas is a mechanical, read-only consequence of the same real transition data the Translator will emit, never a second independently-authored layer that could drift out of sync. That structural fact is exactly what let drag-to-connect, the hub badge, and grammar authoring all integrate with zero conflicts on first build — an external model that moves condition logic out of the diagram would break that property, not just change a preference.

**Also corrected on the record:** the external document's own §11 changelog claims embedded condition/trigger syntax "was tried and explicitly reversed" in this project. That did not happen — the grammar was proposed, built, live-verified with real Playwright interaction, and committed, never attempted-then-rolled-back. Checked against real repo history (the commit above) rather than accepted on the document's own description, same discipline already applied elsewhere in this project to externally-sourced claims (e.g. the Qwen structuring audit). Flagged in `MfilesProperties.md` directly, so a future reader of the external PRD alone isn't misled by it.

## M-Files Flow: transition deletion, cascade delete, and a floating multi-select toolbar (2026-08-19)

Investigated before building, per standing discipline: confirmed M-Files Flow had no way to delete a transition at all (Studio's table was the only place), and confirmed BPMN Standard's real node-delete behavior first — `@xyflow/react`'s own `deleteElements`/`deleteNodes` both cascade-remove connected edges automatically, never block. That answered the scope question before any code was written: M-Files Flow's `deleteState` needed to change to match, not just gain edge deletion alongside its old blocking behavior.

**Built:** real edge right-click hit-testing (`.mflow-edge-hit`, an invisible wider-stroke sibling of each `path.transition`, matching BPMN's own `FlowEdge.jsx` interaction-stroke precedent) wired to `deleteTransition` — Delete + Undo/Redo, mirroring BPMN's real edge menu content minus Edit-label/Duplicate (no inline label-editing surface exists here, and duplicating a transition has no clear meaning). `useWorkflowStore.js`'s `deleteState` gained an optional `{ cascade: false }` param — default unchanged (blocks with the "in use" alert), so **Studio's table stays byte-for-byte identical**; M-Files Flow's single-delete, bulk-delete, and the new floating toolbar all pass `{ cascade: true }`, matching BPMN's confirmed real behavior. Floating multi-select toolbar (`.mflow-selection-toolbar`) built fresh from real `getBoundingClientRect()` over the selection — explicitly **not** a port of React Flow's `NodeToolbar`/`getNodesBounds`, which have no equivalent on this canvas's own hand-rolled pan/zoom.

**Verified live:** edge right-click → Delete removed exactly one transition; deleting a fully-connected state ("Approved," both an inbound and outgoing edge) produced **no alert**, cascaded both connected transitions away with it; Ctrl+click two connected states → toolbar appeared ("2 selected / Duplicate / Delete") → bulk delete cascaded correctly (states + every transition touching either one). Studio's own delete button re-tested against the same connected state afterward — still shows the identical original alert, row count unchanged. Diamond badge, drag-to-connect, and BPMN Standard all re-confirmed unaffected.

## M-Files Flow's palette rebuilt on BPMN's shell pattern — search, categories, rail dividers (2026-08-19)

Checked the real current palette contents first (not assumed): four groups existed (States, a live read-only Layers list, Annotations, an inline Style editor) — only States/Annotations are genuinely placeable-tile categories in BPMN's sense; Layers/Style are live-data/editor surfaces, not tools to place. Also checked BPMN's own palette directly rather than trusting its header comment, which claims "hover-to-expand" — the real code is `expanded = pinned`, a plain click-to-pin toggle, no hover state anywhere. M-Files Flow's palette already used that same mechanism, so there was nothing to port for the expand behavior itself; the real gap was search + multi-category grouping + rail dividers.

**Built:** search bar + three filterable categories (Annotations, Connectors, States — order and reasoning below), rail dividers between non-empty groups. **Hard exclusion held, confirmed both before and after:** no Gateway/Diamond/Hub category, no placeable diamond/hub tile anywhere — diamond/hub stay exactly what they already were, auto-detected read-only badges inside Layers. Connectors' info text describes THIS canvas's real drag-to-connect mechanism (hover reveals a connector dot; drop on empty canvas or the same state cancels, nothing is created) — deliberately not copying BPMN's "auto-creates a connected task" text, since M-Files Flow's own `onUp` handler never does that. Layers and Style deliberately stay outside the search/category machinery and always visible, per operator's explicit call: they show real current data, not tools to find.

**Follow-up polish, same session, each verified live:** Initial State moved to the top of the States group (operator request); category order flipped to Annotations → Connectors → States so **End State is the final tile in the whole palette** and **Status sits at the very top** (operator's explicit "swap location" request, confirmed via DOM order both in the rail and expanded view); State tile's icon changed from a plain "+" to a rounded-rectangle icon with a small **cosmetic-only** diamond corner mark (operator-supplied reference image) — purely a visual hint that this object type can become a diamond automatically; the tile's `onClick` is still `onAddState`, unchanged, confirmed live (state count went 7→8 on click, same as before the icon changed).

## Delete-workflow gap fixed; real localStorage persistence added (2026-08-19)

Two related gaps surfaced from a plain operator question ("can you delete the document"): the workflow tab strip's delete "✕" was hardcoded to hide below 2 workflows (a safety guard with no way to ever act on the last one), and — the more consequential finding — **this app had no persistence anywhere**. Every clear/delete/edit lived only in the Zustand store's memory; a page reload always snapped back to the hardcoded `Document Approval` seed. This is what the operator's "I cleared and it came back" was — not a bug in Clear Canvas, just nothing to survive a reload with.

**Fixed:** `ContextTabStrip.jsx`'s delete button no longer requires 2+ items. `deleteWorkflow` (`useWorkflowStore.js`) no longer no-ops on the last workflow — it replaces it with a genuinely fresh, empty one (same shape `addWorkflow` already creates), so the app is never left with zero workflows. **Added:** the whole store wrapped in zustand's `persist` middleware (already bundled with the installed `zustand` package, no new dependency) → localStorage, `partialize`d to just `workflows`/`activeId`/`users`/`properties`/`rules` — deliberately excluding undo/redo history and transient UI state (hover, command palette), which surviving a reload would be confusing rather than useful. `resetAll()` (the topbar Reset button) still works exactly as before — it overwrites the persisted copy with the original seed on demand.

**Verified live across a real page reload:** deleted the sole "Document Approval" workflow → landed on a blank "New Workflow," canvas stayed mounted; added a state to it, reloaded the whole page, navigated back — both the workflow and the new state were still there. A separate fresh-browser run (no persisted profile) confirmed the seed data still loads correctly for a genuinely new session.

## M-Files Flow: edge-endpoint reconnect (attach/detach) built (2026-08-19)

Operator asked whether an arrow could be moved "like attach and detach." Checked BPMN Standard's real mechanism first: `@xyflow/react`'s native `reconnectEdge()` + `onReconnect`/`edgesReconnectable` — confirmed React-Flow-only, no Mermaid equivalent, same conclusion reached earlier for drag-to-connect's own "magic connector" check.

**Built fresh:** small circular handles sit at each real transition's own start/end point (read back from `path.getPointAtLength(0/len)`, not re-derived from routing math — robust to `orthogonalPath` changing later), dimly visible always rather than hidden-until-hover (discoverability mattered more here than tidiness — no per-node clutter concern the way the node connect-handle has). Grabbing one and dropping it on a different state calls `updateTransition(activeId, transId, { from|to: targetName })`, reattaching that end of the **existing** transition in place — same id, same condition/permissions, nothing recreated. Can't drop onto the state at the other end (no self-loops), same rule drag-to-connect already enforces. Distinct from drag-to-connect's own node-side handle, which only ever creates a brand-new transition — the two coexist without conflict.

**Verified live:** dragged the incoming end of `Submitted → Under Review` onto `Rejected` → became `Submitted → Rejected`; the other five transitions confirmed unchanged by right-clicking each and reading its own context-menu label, before and after. Full regression re-run (diamond badge, drag-to-connect create-path, bulk cascade + toolbar, Studio's blocking delete, BPMN Standard) — all unaffected.

## Side-tab diamond legend — held, per its own gating instruction (2026-08-19)

Asked to build a non-clickable palette legend illustrating diamond-vs-plain states, reusing "the real side-tab diamond component" if that work had landed. Checked first, as instructed: `decision_marker_side_tab.png` (sitting untracked in the project root) is a **design mockup only** — two candidate treatments (Option A: diamond tab attached outside the state's right edge; Option B: diamond badge inside, top-left corner), neither implemented anywhere in `src/` (confirmed via full-repo search), never logged as a started task in `progress.md`. What's actually live today is a third, different treatment — full shape-replacement (the branching state's own `<rect>` is swapped for a `<polygon>`).

**Held, not built**, per the task's own explicit instruction not to build twice. Waiting on the operator to pick a real direction (keep shape-replacement, or switch to mockup Option A/B) before the legend — which needs to illustrate whichever one is real — can be built against it.

## Electron → Translator bridge: feasibility confirmed, real latency measured, no build yet (2026-08-19)

Scoping question for a future "M-Files View" tab: can Electron call the real `ProvisioningAI.Workflow/Translation/` pipeline, and how fast would it actually be? Investigated, did not build.

**Feasibility:** no CLI entry point exists today — `ProvisioningAI.Workflow` is a plain class library, called only from `ProvisioningAI.Tests`. `ProvisioningAI.Api` already project-references it, but is unmodified `dotnet new webapi` scaffolding (the default weatherforecast sample, nothing else) — not running today, and standing it up would collide with the Cacoo Python proxy already occupying port 5000 (confirmed live in `electron/main.cjs`). Electron already has a proven, working bridge pattern it can reuse as-is: `child_process.spawn('powershell.exe', ...)` for the existing test-connection/push/pull-from-vault calls. `PlanFormatter.ToJson(plan)` already exists — the smallest real addition is a ~15-line console wrapper (read Mermaid + sidecar paths from args, call `TranslationPipeline.Translate`, print the existing JSON formatter's output), not a rewrite.

**Real measured latency** (throwaway probe, built and run in scratch space only, discarded after — nothing added to the real repo beyond the standard gitignored `bin`/`obj` of a normal build): timed against MfilesProperties.md's real §6.1/§6.2 sample (7 states, 8 transitions, the same one `TranslationPipelineTests.cs` uses), 5 runs each.
- **Fresh process per call** (`dotnet compiled.dll`, or a published single-file exe): **~250–300 ms** wall-clock, consistent.
- **`dotnet run`, even with `--no-build`**: **~2.0–2.5 s** — MSBuild's up-to-date check alone costs 8–10x more than running the compiled artifact directly. Confirmed trap, avoid regardless of which cadence gets chosen.
- **In-process `Translate()` call itself**, steady-state median over 100 calls: **~0.04–0.10 ms**. Even the very first, JIT-cold call: under 0.5 ms.

**Conclusion, not yet acted on:** essentially all of the ~250ms is CLR/process startup, not the translator — the algorithm itself is effectively free. Recompute-on-open is fine under either model (250ms is imperceptible for an open action). True live/debounced updates would want a persistent warm process (stdin/stdout, not spawn-per-call) to get down to the sub-millisecond numbers above — a materially bigger addition than the CLI wrapper (needs a request/response protocol + lifecycle management), so this is a real fork in scope the operator needs to pick, not a detail to default on.

## Session stopped for the day — resume codeword "word" (2026-08-19)

All six entries directly above are real, live-Playwright-verified (except the bridge investigation, which was measurement/research only — nothing built there to verify). **Nothing has been committed yet** — `git status` shows `App.jsx`, `ContextTabStrip.jsx`, `MFlowCanvas.jsx`, `MFlowPalette.jsx`, and `useWorkflowStore.js` all still sitting as uncommitted working-tree changes, unlike the 2026-08-16 session's clean commit-and-push close. Flagging this explicitly rather than letting it read as "shipped" the way the 2026-08-16 entry did.

**Two real open decisions block further work, both need the operator's own call, neither guessed at:**
1. Which diamond treatment is real (shape-replacement / Option A side-tab / Option B corner-badge) — blocks the legend task.
2. Recompute-on-open vs. persistent-warm-process for the M-Files View tab — blocks scoping that tab at all.

**"word" is confirmed as tomorrow's resume codeword — still the same three-way collision noted since 2026-08-16** (Conformity/M-Files investigation, BPMN Standard/Process Docs, M-Files Flow). Check context before assuming which thread. Full resume detail for the M-Files Flow thread specifically: `recover.md`'s matching 2026-08-19 entry.

## M-Files Flow: Live Translating Split-Screen View built — CLI wrapper, Electron bridge, split-screen layout, LiveTranslationView with debounce (2026-08-20)

Picked up the electron→translator bridge scoped-but-not-built at the end of 2026-08-19. Confirmed cadence up front: permanent split-screen, ~300ms debounce after edits stop, spawn-per-call CLI translation (~250ms) — the persistent-warm-process alternative was explicitly declined as unnecessary complexity for this cadence.

**Step 1 — real CLI wrapper**, `ProvisioningAI.Workflow.Cli` (new console project, added to `ProvisioningAI.sln`, `ProjectReference` to `ProvisioningAI.Workflow`): reads Mermaid text on stdin, calls `TranslationPipeline.Translate()` + `PlanFormatter.ToJson()`, writes JSON to stdout. Verified byte-for-byte against MfilesProperties.md §6.1/§6.2's worked example — 7 states, 8 transitions, the one expected `UNRESOLVED_SCRIPT_REFERENCE` warning, exact match.

**Step 2 — Electron bridge**: `workflow:translate` IPC handler in `electron/main.cjs` (same `spawn()`/stdout-capture shape as the existing PowerShell vault-ops handlers), exposed via `electron/preload.cjs` as `window.workflowTranslator.translate({mermaid})`.

**Step 3 — split-screen layout**: `react-resizable-panels` added as a dependency, deliberately **pinned to 3.0.6, not the newest major (4.x)** — 4.x renamed the whole API (`Group`/`Panel`/`Separator`, `orientation` prop) away from the classic `PanelGroup`/`Panel`/`PanelResizeHandle`/`direction` API this task was scoped against; 3.0.6 is the last version with that API. `MFlowCanvas.jsx`'s existing shell (palette + canvas-area + table-panel) became the left `Panel` (55% default), a new `LiveTranslationView` component the right `Panel` (45%), draggable `PanelResizeHandle` between them.

**Step 4 — `LiveTranslationView.jsx`** (new file, `src/components/mflow/`): three views at first (Flattened/JSON/Validation — a fourth, M-Files Diagram, was added later the same day, see below), `isTranslating` indicator, fade-in keyed to a real new-output-arrived version counter (not to tab-switching, so tab clicks don't replay the fade).

**Real bug found via the user's own live testing, not caught by this session's own Playwright verification the first time — a genuine miss, corrected the same day.** The user reported drawing a new state and not seeing it appear in the translated output. Root-caused with temporary `[DIAG]` logging directly in `MFlowCanvas.jsx`'s translate effect: the exact-correct Mermaid text (including the new state) was being sent on every call, but `ProvisioningAI.Workflow.Translation.MermaidParser.cs` only ever discovers a state's existence from edge endpoints or a literal `state X` declaration line — it has **no support at all** for the `ID : label` aliased-declaration line `useMermaid.js` emits for Mermaid's own rendering (confirmed deliberate, not a gap: MfilesProperties.md §3.5 explicitly documents "one bare declaration per line, no alias/label syntax"). A freshly-placed, not-yet-connected state has no edge referencing it, so it was invisible to the translator until a transition touched it. **Fix, entirely on the M-Files Flow side, not touching the shared/tested `MermaidParser.cs` (24 existing tests) at all:** `useMermaid.js` now also emits a bare `state ID` line alongside the existing `ID : name` label line for every state — `state X` is the parser's own already-supported declaration syntax. Verified live, isolated Electron instance: a brand-new disconnected state now appears in both the flattened view and JSON immediately, canvas rendering unaffected, 0 console errors.

**Step 5 — hover-sync (stretch goal, flagged before attempting per the task's own instruction, then approved and built):** hovering a canvas node highlights its matching block in the Flattened view. Keyed on the *sanitized* id (not the raw label) since that's what `PlannedState.Name` always resolves to given the no-alias-syntax grammar above — matching on the raw label would silently fail to highlight for any multi-word state name. Verified live: hover "Draft" → its flattened-view box gets `.hover`; move away → clears; 0 console errors.

**One accepted, unfixed limitation, decided with the operator rather than guessed at:** multi-word state names (e.g. "Under Review") display as their sanitized id ("Under_Review") throughout every view (Flattened, JSON, and — added later — M-Files Diagram), because of the same no-alias-syntax grammar above. Explicitly decided **leave as-is** rather than either (a) extending `MermaidParser.cs`'s grammar (would contradict MfilesProperties.md §3.5's own documented decision) or (b) a client-side display-only remap (would make the JSON tab misrepresent the translator's actual verbatim output).

**Verification method note, worth keeping:** this session's own Electron+Playwright test instances share Electron's *default* user-data profile with a real, separately-running `electron:dev` session the operator had open — a couple of early diagnostic runs called `localStorage.clear()` against that shared profile before this was noticed. All runs after discovery used an isolated `--user-data-dir` per launch; no runs touched the operator's session again. Flagged directly to the operator at the time.

Full task boundary held throughout (repeated in the two follow-up entries below too, per the operator's own explicit instruction each time): zero Studio files (`CommandCenter.jsx`) opened/read/modified, zero Studio UI interacted with — every verification script only ever clicked "M-Files Flow." `useWorkflowStore.js` was never edited this session (its own working-tree diff predates this session, last real commit 2026-08-16); only its already-exported actions were called.

## M-Files Flow: second connector handle (left edge), mirroring the existing right-edge one (2026-08-20, same day)

`MFlowCanvas.jsx` only. The existing drag-to-connect handle's ~86-line drag/hit-test/cleanup block was refactored into one `wireConnectHandle(side)` function, called once for `'right'` (unchanged behavior) and once for `'left'` (new) — so the two mechanisms can't drift apart over time. No other files touched.

**Verified live, isolated Electron instance:** the primary reported scenario — a state positioned to the left of its target, connected via the new left handle — creates a real transition. The pre-existing right handle re-confirmed unaffected with a fresh pair. Cancel-drag from each handle onto empty canvas confirmed zero dangling transitions. Diamond/hub auto-detect, right-click context-menu info blocks, edge deletion (removes exactly one transition), and bulk-delete (Ctrl+A → Delete) all re-confirmed working. 0 console errors.

**Real, pre-existing bug found and flagged, not silently fixed** — confirmed precisely via `document.elementFromPoint`: once a transition already terminates at a given side of a state, that side's connect-handle circle gets covered by the *edge's own* reconnect-handle circle (`.mflow-edge-endpoint`, added for edge-endpoint-reconnect, §2026-08-19's entry above) — a genuine SVG document-order/z-stacking conflict, not something the left-handle addition introduced (it affects the original right handle identically). Only bites on a handle's *second* use once that exact side already has an edge; first use — including the primary "connect via the new handle" scenario above — works cleanly. Root cause: connect-handles are nested inside their node's own early-positioned `<g>`, while edge-endpoint circles are appended directly to the top-level `<svg>` (always later in document order = painted on top) — a real fix would mean detaching connect-handles from node-local positioning, bigger than "mirror the right handle." **Operator's call: track separately, not fixed now.**

## M-Files Flow: new "M-Files Diagram" tab, set as default; Flattened/JSON/Validation unchanged (2026-08-20, same day)

`LiveTranslationView.jsx` (+ `App.jsx` CSS) only. A fourth tab, rendering the same `TranslationPlan` JSON already powering the Flattened list as an actual SVG schematic — layout and rendering logic ported faithfully from `TranslationPlanRenderer.html`'s own reference renderer (`computeLayers`/`layoutPlan`/`edgeLabel`/`renderMFilesDiagram`, §6.2), translated from imperative DOM-append calls into plain JSX, colors remapped from that file's light-theme palette to this app's own dark-theme CSS variables. Same BFS layering, same shared-lane back-edge routing (cycles route through one lane on the right rather than bowing through neighbors), same label-collision nudging, same skeleton/unparsed-edge treatment (gold `⚠ unparsed: "..."`, matching the existing `.mflow-ltv-chip-warn` gold convention already used elsewhere in this file for warnings). **No diamonds, ever** — every state a plain rectangle regardless of outgoing count, collapse/promote shown as caption text under the box, never a shape change — the same rule the Flattened list already follows, now also true of this rendered view.

Set as the new default tab (`useState('diagram')`); tab bar order: M-Files Diagram, Flattened, JSON, Validation. Flattened's own component and behavior untouched.

**Verified live, isolated Electron instance, screenshot captured:** default-active tab confirmed "M-Files Diagram," SVG renders with real content (6 `<rect>`s for the 6-seed-state workflow), screenshot shows Draft with its initial marker, Closed with the thicker terminal border + "terminal" caption, Under_Review with 2 outgoing edges rendered as a plain rectangle (not a diamond) branching to Approved/Rejected, and the Rejected→Draft back-edge correctly routed through the curved shared lane on the right rather than cutting through Submitted/Under_Review. Switching through all four tabs and back confirmed content unchanged in each. Re-ran the connector-handle regression (both handles, diamond badge) from the entry above — still working. Process Docs (BPMN Standard) re-confirmed rendering clean — not under this task's Studio-only boundary, so checked directly. 0 console errors.

## M-Files Flow: Decision shortcut tile, dashed-automatic-transition verification, hub-at-3+ verification (2026-08-20, same day)

Three independent pieces, `MFlowCanvas.jsx` and `MFlowPalette.jsx` only.

**Part 1 — Decision shortcut palette tile.** New "Shortcuts" category (inserted between the existing Connectors and States categories, so End State stays the palette's final tile — the ordering convention noted in `MFlowPalette.jsx`'s own header comment is preserved, not disturbed). One tile, "Decision": on click, creates one state and two outcome states and wires two real transitions between them — five calls total, all through the exact same `addState`/`addTransition` primitives every other palette tile already uses, `uniqueStateName` reused for collision-safe names ("Decision," "Outcome A," "Outcome B"). The diamond that then appears on the decision state is the pre-existing 2+-outgoing auto-detect firing on genuinely real data — no new rendering path, no directly-placeable diamond object, preserving the file's own long-standing hard exclusion (its header comment was updated to describe this precisely, not left stale). **Verified live:** states +3, transitions +2, both `Decision→Outcome A`/`Decision→Outcome B` pairs present, diamond badge present, right-click menu's diamond info block present.

**Part 2 — dashed rendering for automatic transitions, right side only (verification, not new code).** This logic was actually already built the same day, earlier, as part of the M-Files Diagram tab (`isAutomatic = t.TriggerMode !== 'Manual'` → `strokeDasharray`) — today's task was to *verify* it with a real automatic transition, not build it. Confirmed the left authoring canvas has, and needed, zero changes — no `stroke-dasharray` exists anywhere in `MFlowCanvas.jsx`'s edge-drawing code, matching "left: no change" exactly. Since M-Files Flow's Transitions table is deliberately read-only for `conditions` (grammar authoring is Studio-only, out of bounds for this task), verification used the app's own already-shipped `zustand persist` localStorage format (`provisioningai-workflow-store`) to set one real transition's `conditions` field directly — not a new mechanism, not a Studio interaction, not an edit to `useWorkflowStore.js`, just writing the same JSON shape the app already reads on every load — then reloading. **Verified live:** post-reload, every transition path on the left canvas has `stroke-dasharray: null` (including the one just patched — genuinely unchanged, not universally solid by coincidence), while the right M-Files Diagram tab shows the same patched edge with `stroke-dasharray: "7,5"` alongside other, untouched edges still at `null` — proving the distinction is real and correctly scoped to one side.

**Part 3 — hub badge verification at more than 2 inbound.** Built a decision state ("Outcome A") with **4** real inbound transitions (1 from Part 1's own Decision state, 3 more dragged from fresh states via the connector handles) — deliberately more than the minimum "3+" asked for, to put real distance between this and the "2" already tested in an earlier task. **Verified live:** hub badge present, right-click menu's hub info block reads the literal, correct `"4 incoming from: Decision, New State, New State 2, New State 3"` (no fabrication — read straight from real transition data), Layers palette row for the hub state shows the hub icon. Confirms the pre-existing `inbound >= 2` rule genuinely generalizes past exactly 2, rather than having only ever been exercised there.

**Confirms and broadens the scope of the SVG z-order bug flagged (not fixed) in the previous connector-handle task.** While building the 3-source hub, one connect-handle drag silently no-op'd; `document.elementFromPoint` at that exact screen position resolved to `.mflow-edge-hit` (an edge's wide invisible right-click hit-stroke), not the connect-handle — the *same* underlying cause as the earlier `.mflow-edge-endpoint` finding (both are appended directly to the top-level `<svg>`, always later in document order than a connect-handle nested inside its own node's `<g>`), just a different specific overlapping element this time. Broader than originally characterized: not limited to "a handle's second use once that side already has an edge" — any connect-handle whose screen position happens to fall under *any* rendered edge's path or hit-area is affected, regardless of which state owns which edge. No functional damage either way (the swallowed drag was a clean no-op, zero dangling data) — confirmed by retrying from a repositioned state, which succeeded immediately. Still tracked separately, per the operator's own standing call; not fixed as a side effect of this task.

**Verified regression, same standard as always:** diamond/hub auto-detect (both now re-tested at 2 and beyond-2 respectively), both connector handles (used throughout Part 3), edge deletion (transition count dropped by exactly 1), bulk-delete (Ctrl+A → Delete correctly cleared all states — confirmed with a clean, isolated repro after a busier full-run reported an inconclusive `12→12` readout that turned out to be a test-script artifact of that specific cluttered scene, not a real regression: `Ctrl+A` correctly read "6 states selected," the context menu correctly showed "6 states" with a working Delete button, and Delete correctly removed the States table entirely on a fresh, uncluttered check), all four right-side tabs, BPMN Standard (rendered clean, not under this task's Studio boundary). Zero console errors across every run.

## M-Files Flow: process groups (background highlight + label) and transition names (2026-08-20, same day)

Two more pieces, both requested as quick follow-ups right after the Decision/dash/hub batch above closed. `MFlowCanvas.jsx` only — no other files touched, and critically, **`useWorkflowStore.js` was not edited for either of these**, despite both needing genuinely new persisted fields: `updateState`/`updateTransition` already accept an arbitrary patch object (confirmed by reading their real implementations before building anything — both just spread `...patch` onto the target record with no fixed shape), so a brand-new field can be introduced entirely by calling an existing action with a new patch key. No new store action, no new top-level array, no edit to the shared file.

**Process groups** — a background region highlighting a related set of states, labeled with a process name, confirmed via AskUserQuestion before building (this is the "subgraph/grouping" feature explicitly deferred to its own task when the earlier batch closed): membership is a plain `processGroupId`/`processGroupName` field set on each member state's own record via the existing `updateState` call, assigned through a new "Group" action reusing the *existing* multi-select mechanism (Ctrl+A/shift-click, already built) — no per-state dropdown, no new selection UI. "Group"/"Ungroup" added to both the floating multi-select toolbar and the multi-select right-click menu (`▭ Group` disabled below 2 selected; `▭ Ungroup` appears only when the current selection includes an already-grouped state). Rendering lives entirely in `MFlowCanvas.jsx`'s existing render effect: bounding box recomputed live from real node positions every render (same technique the floating toolbar's own bounding box already uses), background `<rect>` + `<text>` label inserted via `insertBefore(..., svgEl.firstChild)` — the **opposite** DOM-order choice from this file's connect-handles/edge-hit-paths (which need to paint on top of nodes); a group background needs to paint *behind* everything, so it goes in at the very front of the SVG's children instead of being appended at the end.

**Verified live, isolated Electron instance:** grouped 3 states via the floating toolbar, background + "Intake Review" label rendered correctly — confirmed as the literal first two children of the entire `<svg>` (ahead of Mermaid's own `<style>`/`<g>` content), stronger proof of correct z-order than just checking they're "behind nodes." Dragging one member moved the background box live. Nodes stayed fully clickable/selectable through the (pointer-events:none) background. Ungrouping 2 of the 3 members correctly left the background in place (the third member is still grouped) rather than disappearing outright. Full regression (connector handle, tabs, BPMN) unaffected, 0 console errors.

**Testing note:** `window.prompt()`'s native dialog is not reliably interceptable via Playwright's `dialog` event inside this Electron build — a listener attached before triggering it never fired, and no error was thrown either (implying the call returned `null`/`undefined` synchronously rather than opening a real, interceptable dialog). Worked around by stubbing `window.prompt` directly via `page.evaluate()` before triggering any prompt-based action, for both this feature and the one below. See skills.md's matching entry.

**Transition names** — a plain, purely cosmetic `label` field on a transition, set the same way a state gets renamed (`window.prompt`, via the existing `updateTransition` call), added to the edge's own right-click menu (`✎ Edit`, next to the existing `✕ Delete`). Confirmed with the operator up front that this is a *separate* field from `conditions` (the real grammar field — `after()`/`if()`/`script()`/`role()` — that drives `TriggerMode`, the M-Files Diagram tab's dashed rendering, and indirectly the diamond/hub semantics via the translator) rather than exposing `conditions` itself for editing, which would have reversed an earlier, deliberate "Studio-only" design decision from a previous session — a much bigger, more consequential change the operator explicitly did not want. The edge context menu's header line now shows the custom label when set, falling back to the original generic `{from} → {to}` arrow text when it isn't.

**Verified live:** right-clicked `Draft → Submitted`, confirmed the generic arrow label before naming, set it to "Approval Kickoff" via Edit, confirmed the menu shows the new name on re-open, confirmed the Transitions table's own Condition column cell is completely unaffected (still `—`) — proving this genuinely doesn't touch `conditions`/`TriggerMode` at all — then cleared the label and confirmed it correctly reverts to the generic arrow text rather than showing an empty string. Regression (connector handle, tabs, BPMN) unaffected, 0 console errors.

**Studio boundary held across all six of today's M-Files Flow tasks** (the four in the entry above plus these two): zero Studio files (`CommandCenter.jsx`) opened, read, or modified; zero Studio UI interacted with in any verification script; `useWorkflowStore.js` untouched all six times (pre-existing working-tree diff, last real commit 2026-08-16) — every new field introduced today (`processGroupId`, `processGroupName`, transition `label`) went in purely through existing `updateState`/`updateTransition` patch calls, never a store edit. **Nothing from today is committed** — `git status` still shows `MFlowCanvas.jsx`, `MFlowPalette.jsx`, `useMermaid.js`, `App.jsx`, `electron/main.cjs`, `electron/preload.cjs`, `package.json`/`package-lock.json`, the new `LiveTranslationView.jsx`, and the new `ProvisioningAI.Workflow.Cli/` project all as uncommitted working-tree changes.

## M-Files Flow: real bug found and fixed — exponential viewBox/zoom runaway during a node drag (2026-08-20, same day)

Found while running a Fit-button verification the operator asked for — not something anyone had reported broken first, and genuinely surprising: a live drag of one state a moderate distance, followed by clicking "Fit," caused **every** state on the canvas (not just the dragged one) to collapse to unreadable ~7×2px slivers. Root-caused with targeted diagnostic logging placed directly inside `growViewBoxToFit` (temporary `console.log`s, removed once the cause was confirmed — not left in shipped code), not guessed at: the logs showed the viewBox growing **exponentially** across the drag's ~20 intermediate mousemove steps — from ~420×120 units to ~6,459×4,764 units, for a drag that only moved the mouse ~650×500 screen pixels.

**Actual cause:** `.mflow-diagram` is a flex container, and this canvas's own zoom-control code sets the SVG's `style.width` directly via JS (in `growViewBoxToFit`, the zoom effect, and the Fit button) to keep the on-screen scale correct as the viewBox grows. Without an explicit `flex-shrink:0` on the SVG, the CSS default (`flex-shrink:1`) let the flex container silently render the SVG **smaller** than its own requested `style.width` once that width exceeded the container — invisibly, with no error. The JS code had no way to detect this: `toUserDelta()` (the drag handler's screen-to-user-space conversion) reads the SVG's *actual* rendered scale via `getScreenCTM()`, which reflected the clamped-down real size, not the size the code's own zoom bookkeeping assumed. Every mousemove event therefore computed an inflated position delta from that wrong (too-small) scale, which fed a bigger `x,y` into the next `growViewBoxToFit` call, growing the viewBox even further, clamping the render even smaller next frame — a genuine, confirmed-live exponential feedback loop, not a one-time miscalculation.

**Fix:** one line — `flex-shrink:0` added to the `.mflow-diagram svg` CSS rule in `App.jsx` (with an explanatory comment; a first attempt at that comment used backticks for inline-code emphasis and broke the build, since this CSS lives inside a JS template literal in that file and an unescaped backtick prematurely closes the surrounding string — caught immediately by `npm run build`, fixed by removing the backticks, not by suppressing the check).

**Verified live, isolated Electron instance:** repeated the exact same moderate drag that previously produced the runaway — the viewBox no longer grows unboundedly, the dragged state and both untouched states all stayed at real, comfortable, consistent sizes (~42–104px, not slivers) both before and after clicking Fit. Broader regression re-run afterward, since this is a canvas-wide CSS change: both connector handles, diamond auto-detect, process-group background rendering, the transition-name Edit button, edge deletion, bulk-delete, all four right-side tabs, and BPMN Standard all confirmed unaffected. Zero console errors throughout every run.

**Not the same issue as the earlier "translator bridge unavailable" / "stopped working" reports from the same session** — those were, respectively, an expected non-bug (viewing the app in a plain browser tab or VSCode's embedded browser preview, neither of which has Electron's preload bridge) and a stale-hot-reload-state issue in a long-running window (resolved by the operator restarting `electron:dev`). This flex-shrink bug is a separate, real, now-fixed defect in the canvas's own zoom math.

---

## M-Files Flow: second real bug found and fixed — Gateway/hub-feeding automatic transitions silently dropped from the M-Files Diagram tab (2026-08-20, follow-up session)

Found while re-investigating the earlier "dashed-automatic-transition rendering" claim at the operator's explicit request to check the real code path rather than trust the prior verification. A simple, non-hub automatic transition (e.g. `Submitted --[after(3d)]--> Under_Review`) rendered dashed correctly, confirming that path was never broken. But a hub-feeding automatic transition (a source state routing through a 2+-distinct-source Gateway group) never appeared on the M-Files Diagram tab at all.

**Root cause, two parts, both in `useMermaid.js`:**
1. The Gateway/hub declaration loop only ever emitted `gw_i : Gateway ...` — never the companion bare `state gw_i` line the 2026-08-19 fix added for ordinary states. Once any state in the diagram uses explicit declarations, `MermaidParser.cs` stops auto-discovering entirely, so `gw_i` never made it into `DeclaredStates` and was silently absent from the translated plan's own State list — even though its edges still parsed. Every edge touching the hub then failed `MFilesDiagramView`'s `pos[state]` lookup and was dropped outright, not just rendered without a dash.
2. The automatic condition's label was attached to the hub's shared outgoing edge instead of each source's own incoming leg — semantically wrong regardless of bug 1, since a shared outgoing edge can be reached by multiple sources with different (or no) conditions.

**Fix:** add the missing `state gw_i` line; move the condition label onto the incoming leg (`f --> hubId${labelSuffix}`) instead of the outgoing one.

**Verified live, isolated Electron instance:** built a real 2-distinct-source Gateway (`Draft`, `Rejected` → shared group → `Closed`), marked the `Draft` leg automatic (`after(3d)`). Before the fix: `gw_0` absent from `plan.States`, only 6 of 8 real transitions rendered. After: `gw_0` present, `Draft → gw_0` shows `TriggerMode: "AutomaticCriteria"`, and the M-Files Diagram tab visibly renders a dashed line labeled "after(3d)" from Draft into the gateway — confirmed via screenshot. Re-ran the simple non-hub case to confirm no regression (still dashed). Full regression pass (both connector handles, Decision tile, hub badge at 3+ inbound, process groups, all 4 tabs, BPMN Standard) came back clean, zero console errors.

---

## M-Files Diagram tab's solid/dashed rule: confirmed correct, then explicitly overridden to always-dashed by operator decision (2026-08-20, follow-up session)

Two separate, sequential steps in the same follow-up session:

**Step 1 — verification, not a change.** Operator asked to confirm whether an earlier "all lines render dashed uniformly" change had been applied, given the left canvas is always solid with no per-transition distinction. Checked git status/diff and the actual `isAutomatic`/`strokeDasharray` conditional in `LiveTranslationView.jsx` directly — no such change existed. The TriggerMode-based conditional (manual → solid, automatic → dashed) was intact and correct, matching everything verified in the hub-bug fix above. Reported "already correct" and stopped, per the task's own instruction not to touch anything if nothing was wrong.

**Step 2 — deliberate design change, immediately after.** Operator then explicitly requested making the M-Files Diagram tab render every transition dashed regardless of `TriggerMode` — confirmed via AskUserQuestion first (ruling out "this is a bug report" as the actual intent) before implementing. `LiveTranslationView.jsx`'s `isAutomatic` conditional was removed; `strokeDasharray` is now a hardcoded `'7,5'` for every edge on that tab.

**What stayed the same:** the left/authoring canvas — already uniformly solid, unaffected either way. The JSON tab's `TriggerMode` values — still computed and displayed correctly (confirmed live: a manual and an automatic transition in the same output still show `"Manual"`/`"AutomaticCriteria"` correctly); only the M-Files Diagram tab's line-*rendering* stopped reading that field. Flattened/Validation tabs, connector handles, process groups, BPMN Standard — all confirmed unaffected via regression pass, zero console errors.

**This supersedes the 2026-08-20 Decision-tile entry's "confirmed left stays solid, right dashes correctly" framing for the M-Files Diagram tab specifically** — the right side now dashes unconditionally, by explicit operator choice, not because TriggerMode detection stopped working.

---

## End-of-day summary and rollup (2026-08-20)

A consolidated pass over everything from today's M-Files Flow work, cross-checked against the actual current code rather than assumed from memory — several items initially reported for this rollup did not match the codebase and are flagged as such below rather than silently written up as done.

**Confirmed, verified in code and/or live this session:**
1. Split-screen live translator (M-Files Diagram tab), CLI bridge (`ProvisioningAI.Workflow.Cli`), resizable panels (`react-resizable-panels` pinned to 3.0.6) — see this file's "Live Translating Split-Screen View" entry above.
2. Hub-feeding-automatic-transition bug found and fixed in `useMermaid.js` (missing `state gw_i` declaration + misattached condition label) — see the dedicated entry immediately above.
3. Diamond ("Decision") shortcut palette tile — built, verified live: clicking it adds exactly +3 states and +2 transitions.
4. Hub badge verified at 3+ inbound transitions (not just the originally-tested 2) — confirmed live via a fresh 3-source setup, badge rendered correctly.
5. Left-edge connector handle added, mirroring the pre-existing right-edge one via the shared `wireConnectHandle(side)` function — confirmed present in `MFlowCanvas.jsx`.
6. Edge deletion, cascade delete (opt-in `{cascade:true}`, Studio's own blocking guard unchanged), and the floating multi-select toolbar — confirmed present in `MFlowCanvas.jsx`.
7. Palette restructure — search box, categorized sections (including the "Shortcuts" category), rail-divider layout — confirmed present in `MFlowPalette.jsx`. (The "non-clickable diamond/plain legend" piece of this item did NOT check out — see below.)
8. Process-group background/labeling on the left canvas — confirmed present (`.mflow-group-bg`, `processGroupId`/`processGroupName` fields, Group/Ungroup actions), verified rendering live in regression passes.
9. M-Files Diagram tab added as the default tab, alongside the pre-existing Flattened/JSON/Validation tabs — confirmed (`useState('diagram')` default in `LiveTranslationView.jsx`).
10. The solid (left, always) vs. TriggerMode-based rule was confirmed correct, then explicitly overridden to always-dashed on the right — see the dedicated entry immediately above for the full two-step sequence.

**Reported for this rollup but NOT confirmed in the current code — flagged rather than written up as done, per the operator's own explicit instruction:**
- A non-clickable palette legend illustrating diamond-vs-plain states — not found anywhere in `MFlowPalette.jsx`. CLAUDE.md §2.4 already documents this as deliberately deferred pending the diamond-treatment decision (side-tab vs. corner-badge vs. current shape-replacement) — it appears that decision is still unresolved and the legend was never built.
- Validation tab "fixed to always reflect current state" plus a "Clear action" — `ValidationPlan` in `LiveTranslationView.jsx` only ever renders whatever `plan.ValidationIssues` the current translation produced (already naturally current-state-only, keyed by `version`) and has no clear/dismiss button of any kind anywhere in the component.
- Bezier-curve connector rendering on the M-Files Diagram tab "matching real M-Files Admin's actual rendering style" — the only cubic-bezier path in that renderer is the pre-existing back-edge shared-lane routing (already documented, not new today). Ordinary edges are still straight lines; no comment or code references a real M-Files Admin screenshot comparison.
- Studio's nav entry hidden — `src/components/sections.js` still has `id: 'studio', enabled: true`; Studio is fully visible in the nav bar in every screenshot taken this session.

These four are genuinely open questions, not erased from the record: they may have landed in a different session/worktree/uncommitted stash, or may simply not have been built yet. Next session should resolve which, rather than assume either way.

**Next session's plan (also recorded in CLAUDE.md §2.4):** build complex, multi-diamond scenarios on the left authoring canvas — several branching states, mixed manual/automatic transitions, at least one 3+-inbound hub — to stress-test the translator and M-Files Diagram rendering under real complexity, since everything verified so far (including both bugs found today) used simple, minimal examples.

---

## React Flow Pro workflow designer — full 17-example inventory, feature matrix, and locked v1 architecture (2026-08-21)

**Scope: a from-scratch architecture plan for a new React Flow Pro-based workflow designer, built by systematically inventorying all 17 downloaded Pro example bundles in `React_Flow_Pro/` rather than ad hoc borrowing.** Distinct from — and broader than — the BPMN Standard canvas's earlier ad hoc React Flow Pro borrowing (2026-08-13/14 entries above); this pass treats the full 17-bundle set as the actual source-of-truth library rather than reaching into it piecemeal per task. No integration code written yet; this entry records the planning phases only.

**Phase 1 — Inventory.** All 17 zips extracted to `React_Flow_Pro/_inventory/` (numbered `01_auto-layout-pro-example` through `17_undo-redo-pro-example`), structurally intact — `package.json`/`README.md`/non-empty `src/` present in every folder. Per-project summary delivered (purpose, dependencies, state management approach). One flag caught and resolved: `14_selection-grouping-pro-example`'s `package.json` lists a dead `reactflow@^11.10.1` dependency alongside the real `@xyflow/react@^12.7.0` — confirmed via grep that nothing in `src/` actually imports from the legacy package (`App.tsx`/`main.tsx`/`GroupNode.tsx`/`SelectedNodesToolbar.tsx`/`useDetachNodes.ts` all import from `@xyflow/react` only) — pure v11→v12 migration leftover, safe to ignore when porting.

**Phase 2 — Feature matrix + official gap check.** Built a feature-by-feature matrix (palette/drag-drop, custom node types, custom edge types, minimap, controls/toolbar, node grouping/subflows, auto-layout, undo/redo, save/load, collaborative editing, helper lines, computing node dimensions, server-side image export) across all 17, then cross-checked against the live `reactflow.dev/examples` index (fetched via the `/browse` skill per this machine's global instruction to use it for all web browsing — not recalled from memory). Key findings:
- **Save/load has zero coverage in any of the 17** — confirmed via grep across every project (`toObject()`, `localStorage`, `IndexedDB`, `JSON.stringify` of nodes/edges/flow: no hits anywhere). The one matrix feature genuinely absent from the Pro example set. reactflow.dev's own free (non-Pro) "Save and Restore" example is the correct reference pattern for it.
- React 19 usage: only `02_collaborative-pro-example` and `08_freehand-draw-pro-example` pin `react@^19.1.0` (the other 15 are all on `^18.2.0`–`^18.3.1`). `08_freehand-draw` has no load-bearing React-19-only API anywhere in `src/` (checked for `useOptimistic`/`useActionState`/`useFormStatus`/`use()` — zero matches) — safe to target React 18 with no rewrite. `02_collaborative` genuinely depends on `useOptimistic` (React 19-only) in `src/components/nodes/CheckboxNode.tsx` and `TextNode.tsx`, used for optimistic UI updates on collaborative node edits — a real, load-bearing dependency, not just a version pin.
- State management split across the 17: Zustand-backed (02, 05, 09, 10, 15), plain `useState`/`useNodesState`/`useEdgesState` (01, 03, 06, 07, 08, 11, 12, 14, 17), fully uncontrolled/`defaultNodes`+`defaultEdges` (04, 13, 16).
- `15_server-side-image-creation-pro-example` is the only one of the 17 requiring a server component (Express + Puppeteer backend, own `Dockerfile`/`fly.toml`) — every other example is a pure client SPA (`npm run dev` + Vite).
- Full advertised-Pro-feature gap list recorded against the live docs (Context Menu, Validation, Connection Events, Computing Flows, Reconnect Edge, Floating Edges, Multi Connection Line, Dark Mode, Tailwind, Lasso Selection, Eraser Tool, Node Collisions, Horizontal Flow, and others — ~30 items total) — not pursued for v1 porting, recorded for future reference only, not part of the feature-source table below.

**Phase 3 — Recommendation + locked v1 architecture.** Per-feature source-of-truth picks made using the stated priority (official reactflow.dev pattern first where one exists and the 17 have no coverage; otherwise the cleanest of the 17 per the Phase 2 notes) — in practice this labels only save/load as "official pattern," since every other matrix feature is already covered by an official Pro-tier example within the 17 (these zips are the actual Pro example downloads, not third-party reimplementations).

**Locked decisions:**
- **State management:** single Zustand store, one slice per feature area, mirroring `02_collaborative`'s multi-slice pattern (`store/{flow,cursor,connection}-slice.ts` + composition). Every ported feature — including every useState/uncontrolled one in its source example — gets rewritten into a Zustand slice during its own port, not left as bare local state.
- **React version:** 18 project-wide. `02_collaborative`'s `useOptimistic` usage in `CheckboxNode.tsx`/`TextNode.tsx` gets rewritten to `useState` + manual rollback-on-reject during that feature's port (operator decision, confirmed 2026-08-21 via AskUserQuestion after the dependency was flagged rather than silently downgraded — the other two options offered were "isolate as a React 19 module" and "defer the decision until that module is ported").
- **Save/load:** new feature, built fresh from reactflow.dev's own free "Save and Restore" pattern (`toObject()`/`setNodes`+`setEdges`), JSON file export/import to start (file save/open), explicitly not a database.
- **Edge types — three selectable options, not one "winner":** `04_dynamic-layouting-pro-example`'s simple `PlaceholderEdge`/`WorkflowEdge` pair (default, zero extra deps beyond `d3-hierarchy`/`d3-timer`), `05_editable-edge-pro-example`'s draggable-control-point editable edge (zustand-backed), and `10_libavoid-edge-routing-pro-example`'s WASM-based obstacle-avoiding libavoid edge — all three registered in `edgeTypes.ts` as selectable, none baked in as the sole default. **Confirmed (2026-08-21): `10_libavoid` needs no special bundler plugin** — its `vite.config.ts` is plain `@vitejs/plugin-react`, nothing WASM-specific. The ~528KB `libavoid.wasm` binary (`src/assets/libavoid.wasm`) is resolved via Vite's native `?url` asset-import suffix (`import wasmUrl from '../assets/libavoid.wasm?url'`) and loaded at runtime via `AvoidLib.load(wasmUrl)` (`src/hooks/useLibavoid.ts`). Only real requirements when porting: copy the `.wasm` binary into the target project's assets, and gate the libavoid edge type behind that async `AvoidLib.load()` init completing — it's the only one of the three edge types with an async-init gate; the other two (04, 05) are synchronous like every other ported feature.
- **Collaboration (`02_collaborative`) and server-side image export (`15_server-side-image-creation`) are v1 features, not deferred.** Both are explicit selling points against Cacoo and ship as part of the base app (operator decision, 2026-08-21). **This explicitly reverses this file's own 2026-08-14 "Deferred/rejected tracker" entries** above — "Collaboration features (deferred)", "libavoid obstacle-avoid routing (deferred pending feasibility)", and "Server-side image export (deferred; no backend infra in place)" are all superseded by this decision. Flagged here per that tracker's own "durable, do-not-reopen silently" instruction, so the reversal is visible in the record rather than quietly overwritten. Each still requires its own running infrastructure (a y-websocket server for collaboration; the Express+Puppeteer service for image export) — that's a deployment/README requirement documented inside each feature's own folder, not a scoping qualifier that makes either feature optional.

**Feature-source matrix (final, corrected 2026-08-21 — row 5 originally read "check bundler config for WASM asset" before the libavoid build-requirements check above was run):**

| # | Feature | Port source | Notes |
|---|---|---|---|
| 1 | Palette / drag-drop node creation | 16_shapes-pro-example | Only example with a real sidebar-to-canvas palette; zero extra deps |
| 2 | Custom node types | 16_shapes-pro-example | Continues from the palette port; zero extra deps |
| 3 | Minimap | 16_shapes-pro-example | Only example with a custom minimap-node renderer (`components/minimap-node/index.tsx`) |
| 4 | Controls / toolbar | 16_shapes-pro-example (per-node toolbar) + 14_selection-grouping-pro-example (multi-select floating toolbar) | Two distinct toolbar patterns, both ported |
| 5 | Custom edge types (libavoid/WASM) | 04_dynamic-layouting-pro-example (default pair) + 05_editable-edge-pro-example + 10_libavoid-edge-routing-pro-example | confirmed — no bundler plugin needed, Vite `?url` import resolves the .wasm binary; requires copying the binary into assets + gating the edge type behind `AvoidLib.load()`'s async init |
| 6 | Node grouping / subflows | 14_selection-grouping-pro-example (primary) + 12_parent-child-relation-pro-example (attach/detach gesture) | Both ride React Flow's built-in `parentId` subflow mechanism |
| 7 | Auto-layout | 01_auto-layout-pro-example | 3 engines (dagre/elkjs/d3-hierarchy) via one shared `useAutoLayout` hook. **Superseded 2026-08-21 — see the follow-up entry below: the target (Process Docs) already has a real dagre port of this (`bpmnAutoLayout.js`); relocate/refactor it, don't re-port.** |
| 8 | Undo/redo | 17_undo-redo-pro-example | Snapshot-stack (`useUndoRedo.ts`), rewritten into a Zustand slice during porting. **Superseded 2026-08-21 — see the follow-up entry below: the target already has a real snapshot-based port of this (`useBpmnStore.js`); relocate/refactor it, don't re-port.** |
| 9 | Save/load (serialization) | official pattern — reactflow.dev "Save and Restore" | Only feature with zero coverage in the 17; built fresh |
| 10 | Collaborative editing | 02_collaborative-pro-example | v1 feature; `useOptimistic` rewritten to `useState`; requires a running y-websocket server |
| 11 | Helper lines | 09_helper-lines-pro-example | Zustand-backed alignment/snap computation (`useHelperLines.tsx`) |
| 12 | Computing node dimensions | no standalone port — implicit inside 01_auto-layout-pro-example's layout hook | None of the 17 isolates this as its own pattern |
| 13 | Server-side image export | 15_server-side-image-creation-pro-example | v1 feature; requires the Express+Puppeteer service running |

**Target folder structure locked (feature-based, not type-based):** `src/{app,store,features/{palette,nodes,edges,minimap,toolbar,grouping,layout,history,helper-lines,save-load,collaboration,export-image}}` — full annotated tree with per-folder port sources: CLAUDE.md's matching 2026-08-21 Decision entry (§2.3). **Superseded in part 2026-08-21 (same day) — see the follow-up entry immediately below: this tree's `layout/`, `history/`, and porting-priority order are corrected there, plus a new `bpmn-io/` folder is added.**

**Not yet done, as of this entry:** no integration code written. **Superseded — see the follow-up entry immediately below for the corrected porting order** (a Phase 0 check on the actual swap target changed the sequence materially: save/load moved up, and two "new ports" turned out to be relocations of already-working code instead).

---

## Process Docs canvas swap — Phase 0 findings and revised porting plan (2026-08-21, follow-up)

**Clarification that reframes the whole plan above: this is a swap of the Process Docs tab's canvas internals, not a new tab or a green-field build.** The tab itself — `id: 'bpmn'`, label "Process Docs," its position in the nav (Studio | M-Files Flow | Process Docs | Discovery | Docs | Copilot), its route, `sections.js`'s registry entry — stays exactly as-is, untouched. Only what `AppShell.jsx` mounts for it (`BpmnCanvas.jsx` and its component tree) gets replaced/refactored.

**Confirmed a clean, isolated swap before touching anything:** grepped `MFlowCanvas.jsx` and Studio's `CommandCenter.jsx` directly for any BPMN reference — `MFlowCanvas.jsx` has exactly one hit, a comment explaining a UX-parity decision, not an import; `CommandCenter.jsx` has zero hits. `BpmnCanvas.jsx`/`useBpmnStore.js`/`src/components/bpmn/*`/`src/utils/{bpmnAutoLayout,bpmnModdle,bpmnPools,bpmnHelperLines}.js` share zero imports, code, or data with Studio or M-Files Flow. Discovery/Docs/Copilot are still gated placeholders (`enabled: false` in `sections.js`). No coordination needed with any of those three.

**Phase 0 inventory of the current Process Docs implementation found it materially more built than a fresh 17-example port would assume:**
- Already on `@xyflow/react@12.11.3` — the exact modern package this whole architecture targets. `react-flow-renderer@10.3.17` is also still listed in `package.json` but confirmed unused (zero imports anywhere in `src/`, already flagged as dead in the 2026-08-13 BPMN entry above) — unrelated to this swap, nothing to migrate away from.
- **Undo/Redo is a real, working port already** — `useBpmnStore.js`'s snapshot-based `takeSnapshot`/`undo`/`redo`/`history` (capped at 100 entries), wired to toolbar buttons, Ctrl+Z/Ctrl+Shift+Z, and the right-click context menu. Adapted directly from `17_undo-redo-pro-example` (confirmed via this codebase's own citing comments in `useBpmnStore.js`), with two deliberate, documented deviations already made from the source (fixed an inverted `canUndo`/`canRedo` in the original example, added an input-focus guard the demo never needed). Not a stub.
- **Auto-arrange is a real, working port already** — `autoArrange` in `BpmnCanvas.jsx` calls `layoutWithDagre` (`src/utils/bpmnAutoLayout.js`, 33 lines, dagre-based), adapted from `01_auto-layout-pro-example`, simplified in one documented way (builds a fresh graph per call instead of reusing/pruning one module-level graph). Not a stub.
- **Export BPMN / Import are real, working features with no relationship to any of the 17 examples** — `src/utils/bpmnModdle.js` (239 lines), a genuine `bpmn-moddle`-backed round trip: export serializes to real schema-validated BPMN 2.0 XML (validated via `fromXML` before download, not just "didn't throw"), import parses a real `.bpmn`/`.xml` file back into nodes/edges. Confirmed via `handleExportBpmn`/`handleImportBpmnFile` in `BpmnCanvas.jsx` and the animate toggle's real `<animateMotion>`/`<mpath>` SVG wiring in `FlowEdge.jsx` (grepped directly, not assumed from the toolbar button's title text alone).
- **Zero persistence exists today.** `useBpmnStore.js`'s `nodes`/`edges` start from a hardcoded `initialNodes`/`initialEdges` starter sketch (the visible Start → Receive invoice → Route to approver/Return to vendor → End example, lines 110–126) on every load. The store's own comment states it directly: not persisted anywhere, no localStorage, no persist middleware — a real page reload always resets to the same static starter. **There is no saved user diagram to migrate** — only the starter sketch's own content needs re-seeding into whatever replaces the store.

**Revised locked decisions, superseding the corresponding bullets in the entry above:**
- **State management:** `useBpmnStore.js` becomes THE single store this architecture describes — refactored in place into one slice per feature area as each feature is relocated or ported. Not a second, parallel store built fresh alongside it.
- **Undo/redo and auto-layout:** relocate/refactor the existing real implementations into `features/history/` and `features/layout/` respectively, converting each into the locked slice pattern. Do NOT re-port from `17_undo-redo-pro-example` or `01_auto-layout-pro-example` a second time — the existing code is already correct and already cites those sources.
- **BPMN export/import:** relocate `bpmnModdle.js` as-is into a new `features/bpmn-io/` folder — no rewrite, just moved and rewired to the new store shape once it exists. Has no 17-example source and needs none.
- **Save/load priority:** moved up to position 3 in the porting order (right after palette and custom nodes), not landing late — Process Docs' confirmed zero-persistence gap is its single biggest real deficiency today, not a nice-to-have deferred to the end.
- **Re-seed requirement:** the existing 6-node invoice starter sketch becomes the refactored store's default/example content, so Process Docs doesn't open empty on first load post-swap.

Full corresponding CLAUDE.md update: §2.3's matching 2026-08-21 Decision entry, same day.

---

## Palette also caught as an already-real port — corrected at the start of actual porting (2026-08-21, same day, second follow-up)

**Starting step 1 of the revised order above (palette from 16_shapes) surfaced the same situation Phase 0 already found for Undo/Redo, Auto-arrange, and Export/Import — just missed the first time because Phase 0's checklist scoped the toolbar specifically, not the palette.** Before writing any new palette code, read `src/components/bpmn/BpmnPalette.jsx` directly rather than assuming "new" was still correct.

**Finding: `BpmnPalette.jsx` is already a real, working palette, and it is already a port of `16_shapes-pro-example`.** Its own comment states it directly: adapted from 16_shapes' `sidebar-item.tsx`/`App.tsx` drag-drop pattern (`setData` on dragstart, `screenToFlowPosition` + `setNodes` on drop). It's materially more built than 16_shapes' own bare, undecorated draggable shape icon — `BpmnPalette.jsx` has categorized sections (Events/Activities/Gateways/Connectors/Containers), a search box, hover-expand-with-pin behavior, and both drag-and-drop and click-to-add, all wired into `BpmnCanvas.jsx`'s `onDrop`/`onDragOver`. Confirmed via direct file read, not assumed from the file's existence alone.

**Decision, confirmed with the operator via AskUserQuestion before proceeding (three options offered: relocate/refactor like the toolbar items; re-port from the zip anyway and accept losing categories/search/pin; or a hybrid keeping the shell but borrowing 16_shapes' Shape-preview rendering) — relocate/refactor, same treatment as Undo/Redo and Auto-arrange:** move `BpmnPalette.jsx` into `features/palette/`, convert its local `useState` (search) into the locked slice pattern, keep its existing categories/search/pin/drag-drop behavior intact. Do not re-port from `16_shapes-pro-example`'s `sidebar-item.tsx` a second time.

**General lesson that immediately proved itself again:** before treating any remaining "new" item in the feature-source matrix as a fresh port, check whether Process Docs' existing code already covers it first.

---

## Full re-audit against the CLAUDE.md architecture — 10 of 14 features already exist (2026-08-21, same day, third follow-up)

**Checking node types (the next item after palette) before writing new code caught a fourth item in a row that was already real** — `GatewayNode.jsx`/`TaskNode.jsx`/`EventNode.jsx`/`PoolNode.jsx` (102/53/41/46 lines respectively), all wired into `BpmnCanvas.jsx`'s `nodeTypes`, none stubs. Rather than keep catching this one item at a time — palette, then undo/redo, then auto-layout, then export/import, now node types — did one complete pass checking every remaining architecture feature against the actual current codebase before writing anything else, per the operator's explicit direction (offered three options via AskUserQuestion: full re-audit first; keep checking item-by-item; or skip straight to inference — full re-audit chosen).

**Result — full feature-by-feature audit:**

| Feature | Status | Evidence |
|---|---|---|
| Palette | real | `BpmnPalette.jsx` |
| Custom node types | real | `GatewayNode`/`TaskNode`/`EventNode`/`PoolNode` |
| Undo/redo | real | `useBpmnStore.js` snapshot history |
| Auto-layout | real | `bpmnAutoLayout.js` (dagre) |
| Export/Import BPMN | real | `bpmnModdle.js` (real bpmn-moddle schema validation) |
| Minimap | real, adequate | `<MiniMap nodeColor={minimapNodeColor}/>` in `BpmnCanvas.jsx` — color-codes by type via a function rather than a custom minimap-node component (16_shapes' technique); functionally equivalent, different implementation approach |
| Toolbar (multi-select + per-node) | real | `SelectedNodesToolbar.jsx` (66 lines) + inline `NodeToolbar` |
| Grouping/subflows (Pool) | real | `bpmnPools.js` (113 lines), `groupSelectedIntoPool` |
| Helper lines | real | `bpmnHelperLines.js` (88 lines) + `HelperLines.jsx` (22 lines) |
| Edges — base type + connector style | real | `FlowEdge.jsx` (172 lines) — orthogonal/straight/curved via React Flow's own path functions, inline label/comment edit, animated dot |
| Edges — editable control points (05) | **genuinely absent** | No draggable-control-point routing anywhere in this codebase |
| Edges — libavoid WASM routing (10) | **genuinely absent** | No libavoid/WASM anywhere in this codebase |
| Save/load | **confirmed absent** | No persistence anywhere — hardcoded starter only, no localStorage |
| Collaboration | **not built** | No yjs/y-websocket/multiplayer anything |
| Server-image export | **not built** | No Express/Puppeteer service |

**10 of 14 architecture features already exist and are real** — only relocation into `features/` + conversion to the locked slice pattern is needed for those, no new capability. **Only 4 things are genuinely new work:** save/load; two additional edge types layered onto the existing `FlowEdge` (05 draggable control points, 10 libavoid WASM routing — additions, not replacements); collaboration; server-image export.

**Sequencing decision, confirmed with the operator via AskUserQuestion:** save/load needs a stable store shape to serialize against. Refactor the 10 existing real features into the slice architecture first, THEN build save/load against the finished store shape — avoids wiring save/load twice (once against `useBpmnStore.js`'s current shape, again after the slice refactor lands). This pushes save/load later than the original "build it early" plan, but only after everything else is already done, not indefinitely deferred.

**Final porting order (supersedes all three prior orderings above — this is the one to build against):**
...
11. Save/load (`features/save-load/`) — Process Docs' confirmed biggest gap, built once the refactor is done
12. Two additional edge types layered into `features/edges/`: 05's draggable-control-point editable edge, 10's libavoid WASM obstacle-routing edge
...
**General lesson, now proven three times over:** treat any "new" label anywhere in this architecture as provisional until checked against the current `src/components/bpmn/`/`src/utils/` tree. Three separate rounds of checking (toolbar in Phase 0, palette at the start of porting, this full audit) each caught something the previous pass had assumed needed a fresh port.

Full corresponding CLAUDE.md update: §2.3's matching 2026-08-21 Decision entry, same day.

**Phase A, step 1 — done and live-verified (2026-08-21, same day).** `BpmnPalette.jsx` relocated to `src/features/palette/BpmnPalette.jsx`; its local `useState(search)` and `BpmnCanvas.jsx`'s local `palettePinned` state both converted into a new `src/features/palette/paletteSlice.js`, composed into `useBpmnStore.js` via the slice-combination pattern (`...createPaletteSlice(set, get)`), matching `02_collaborative-pro-example`'s own multi-slice approach. `BpmnCanvas.jsx` updated: import path fixed, local `palettePinned` state and its now-redundant `pinned`/`onTogglePinned` props removed (the palette reads its own state directly from the store now, same direct-store-read pattern already used for `animateFlow`/`businessView`/`connectorStyle`). `npm run build` clean. Live-verified in the real running app (not just build-clean): pin/expand toggle works, search correctly filters to the Gateways category on "gate", click-to-add still creates a real node (confirmed via Undo becoming enabled and the node appearing), Undo/Redo round-trips the test node cleanly, zero console errors through the whole sequence. Full corresponding CLAUDE.md note: same day.

**Phase A, step 2 — done and live-verified (2026-08-21, same day).** `GatewayNode.jsx`, `TaskNode.jsx`, `EventNode.jsx` (`StartEventNode`/`EndEventNode`), and `PoolNode.jsx` relocated verbatim to `src/features/nodes/` — no content changes needed, since all four already imported only from `store/`/`utils/` at a relative depth (`../../`) that's identical whether the file lives under `components/bpmn/` or `features/nodes/`. New `src/features/nodes/nodesSlice.js` extracts the `nodes` array, all spawn-position helper logic, and every node CRUD action (`onNodesChange`, `setNodes`, `addTask`/`addStart`/`addEnd`/`addGateway`/`addSubProcess`/`addPool`, `updateNodeLabel`, `duplicateNode`, `duplicateNodes`, `deleteNodes`) out of `useBpmnStore.js`'s previously-flat structure, composed in via the same slice pattern as `paletteSlice`. `makeId` extracted into a new tiny `src/store/makeId.js` specifically to avoid a circular import (`useBpmnStore.js` → `nodesSlice.js` → back to `useBpmnStore.js`), with `useBpmnStore.js` re-exporting it so `BpmnCanvas.jsx`'s existing `import { useBpmnStore, makeId } from '../store/useBpmnStore'` keeps working unchanged. Scope held deliberately narrow: `groupSelectedIntoPool` (wrapping an existing selection into a pool) stayed in the root store rather than moving here, since that's grouping-feature interaction reserved for its own later step (6), not node creation — `addPool` itself (creating one plain pool) did move, same category as `addTask`/`addGateway`. `deleteNodes` still cascades into `s.edges` directly from within the nodes slice — confirmed safe, since Zustand's combined store gives every slice's actions access to the full state regardless of which slice originally defined them. `npm run build` clean. Live-verified in the real running app: all four node types render identically to before (Start/End events, Task, the exclusive-gateway diamond, and a freshly-created Pool's left-edge label strip all confirmed via screenshot); click-to-add tested for both Task and Pool (new nodes appeared correctly, Undo became enabled); Undo/Redo round-tripped cleanly in both directions across two separate test nodes; zero console errors through the entire sequence. Drag-and-drop node creation not separately exercised in headless testing (HTML5 drag-and-drop is unreliable to simulate via CDP) — reasoned instead of assumed: `BpmnCanvas.jsx`'s `onDrop` handler was untouched by this relocation and calls the exact same `onAddTask`/`onAddGateway`/etc. props (now backed by the relocated slice) that click-to-add already exercised, so the same code path is confirmed either way.

**Phase A, step 3 — done and live-verified (2026-08-21, same day), preceded by a confirmation gate the operator explicitly required before touching anything.** Checked `bpmnModdle.js` directly first (same discipline as step 2): confirmed `FlowEdge.jsx`'s `label` prop is real BPMN structural data — `toBpmnDefinitions` writes `name: edge.label || undefined` onto every `bpmn:SequenceFlow`, and `importBpmnXml` reads it back the same way — while `edge.comment` (FlowEdge's own internal annotation field) is referenced nowhere in that file. So `FlowEdge.jsx` is genuinely BPMN-semantic, same as the four node types in step 2, and the relocate-not-replace call applied here too — confirmed with the operator before proceeding, per their explicit gate.

`FlowEdge.jsx` relocated verbatim to `src/features/edges/FlowEdge.jsx` (no content changes needed — same relative-depth-preserved reasoning as step 2's node components). New `src/features/edges/edgesSlice.js` extracts `edges`, `activeEdgeToolbarId`, `onEdgesChange`, `setEdges`, `onConnect`, `reconnectEdge`, `updateEdgeLabel`, `updateEdgeComment`, `duplicateEdge`, `deleteEdge` out of `useBpmnStore.js`, composed in via the same slice pattern. Scope held narrow, same precedent as step 2: `animateFlow`/`connectorStyle` (whole-canvas display toggles FlowEdge also reads directly) stayed in the root store rather than moving into the slice — same reasoning nodesSlice.js used to leave `businessView` in root. `npm run build` clean.

**Live-verified against all five points the operator specifically asked for, with real evidence, not just visual confirmation:**
1. **Label ↔ SequenceFlow.name round-trip** — edited the "invalid" edge's label to `invalid-relocated-test` via FlowEdge's own floating toolbar, exported, and intercepted the actual Blob content (overrode `URL.createObjectURL` to capture the XML rather than trusting the download UI). Confirmed the exact line: `<bpmn:sequenceFlow id="Flow_bp-e4" name="invalid-relocated-test" sourceRef="Node_bp-gate" targetRef="Node_bp-reject" />`. Then fed that same XML back through Import BPMN (via a real file upload, not a mock) — canvas correctly showed "Imported 6 elements — 0 schema warnings" with the label restored on the edge, confirming both export and import directions.
2. **Comment stays internal-only** — added a comment (`internal-note-not-exported`) via FlowEdge's Comment panel, then confirmed programmatically (`.includes()` on the captured export string, not eyeballing) that it does not appear anywhere in the exported XML, while the label does.
3. **Connector-style switching** — switched Orthogonal → Curved via the palette's segmented control; screenshot confirms all 6 edges immediately re-rendered as smooth bezier curves. Switched back to Orthogonal afterward, zero errors either direction.
4. **Animated dot** — toggled Animate on; confirmed via DOM query (not just visual impression, since a static screenshot can't show motion) that exactly 6 `.edge-flow-dot` elements exist with 6 `<animateMotion>`/6 `<mpath>` children, matching all 6 edges. Toggled back off cleanly.
5. **Undo/redo** — duplicated an edge (6→7), Undo (7→6), Redo (6→7), Undo again (7→6) — every step confirmed by counting real `.react-flow__edge` DOM nodes, not assumed from button state alone.

Zero console errors through the entire test sequence, every step. Per the operator's explicit instruction, **05_editable-edge and 10_libavoid-edge-routing are NOT added yet** — that's a separate, deliberately deferred follow-up pass once this relocation was confirmed clean, which it now is.

**Save/load — done and live-verified (2026-08-21, same day), moved up ahead of the remaining Phase A relocations (operator decision).** Rationale: nodes and edges are already in their final slice shape after steps 2–3, so building save/load now — rather than waiting for Phase B as originally sequenced — carries none of the "wire it twice" risk the original ordering was protecting against.

New `src/features/save-load/saveLoadSlice.js` (`saveToFile`, `loadFromFile`), composed into `useBpmnStore.js` via the same slice pattern. Follows reactflow.dev's own free "Save and Restore" pattern precisely: `toObject()` (called in `BpmnCanvas.jsx`, the only place `useReactFlow()` is reachable — a plain Zustand action can't call a React hook) captures `{nodes, edges, viewport}`; the slice's `saveToFile` triggers a real Blob/`<a download>` JSON file download, matching Export BPMN's own established UI pattern (no database, no localStorage auto-persistence, per the locked architecture). `loadFromFile` reads a real uploaded file, parses it, and restores via the existing `setNodes`/`setEdges` actions — made undoable via `takeSnapshot()` first, matching `handleImportBpmnFile`'s own precedent exactly rather than `resetAll`'s history-clearing behavior. New "💾 Save"/"📂 Load" toolbar buttons added next to Export/Import BPMN in `BpmnCanvas.jsx`. `npm run build` clean.

**Live-verified against every point requested, with real evidence:**
- **Starter sketch re-seed confirmed unaffected** — a fresh page load (real navigation, not SPA routing) still shows the plain 6-node starter sketch; adding the slice didn't change `nodesSlice.js`/`edgesSlice.js`'s own `initialNodes`/`initialEdges` defaults.
- **Full save → reload → load cycle**, not just save-then-immediately-load: added a 7th node ("SaveLoad Verification Task"), saved to file (captured the actual Blob content via a `URL.createObjectURL` override, not just trusting the download UI — confirmed the JSON: 7 nodes, 6 edges, a real `viewport: {x,y,zoom}` from `toObject()`, the renamed task present). Did a genuine full page reload — confirmed it reset to the plain 6-node starter (no auto-persistence, exactly as intended). Uploaded the saved file via the real Load button — confirmed 7 nodes restored, the renamed task and its exact position back, viewport visibly matching the saved pan/zoom, status bar reading "Loaded from file."
- **Undo/redo on the load itself** — Undo took it back to the pre-load 6-node state (confirming `loadFromFile`'s `takeSnapshot()` makes the whole load one undo step, not two), Redo restored the 7-node loaded state again.
- **BPMN export/import re-verified with the loaded/modified diagram** — exported after the save/load round-trip and confirmed the same node id (`ewzxlkk`, carried through the whole save→reload→load chain) still produces correct real BPMN XML (`<bpmn:task id="Node_ewzxlkk" name="SaveLoad Verification Task" />`), then re-imported that exact XML through the existing Import BPMN button — "Imported 7 elements — 0 schema warnings," label round-tripped correctly. Confirms save/load and BPMN export/import don't interfere with each other despite touching the same node/edge data.
- Zero console errors through the entire sequence — starter check, modify, save, reload, load, undo, redo, BPMN re-export, BPMN re-import.

**Phase A, steps 4 and 5 — done and live-verified together (2026-08-21, same day): Undo/Redo relocated into `features/history/historySlice.js`, Auto-arrange into `features/layout/` (`layoutSlice.js` + the relocated `bpmnAutoLayout.js` utility, moved out of `src/utils/`).** Same relocate-not-report discipline as every prior step — both were already real, working ports (17_undo-redo and 01_auto-layout respectively), confirmed via this codebase's own citing comments before touching anything. `MAX_HISTORY = 100` and the snapshot-based `{nodes, edges}` history model preserved exactly, byte-for-byte logic, just moved. `autoArrange`'s `fitView()` call — a `useReactFlow()` hook method, unreachable from a plain Zustand action — stayed in `BpmnCanvas.jsx`, which now just calls the new `autoArrange` store action and then `fitView()` itself; same split pattern already established for save-load's `toObject()`/`setViewport`. `npm run build` clean.

**Live-verified against every point requested:**
- **All three undo/redo trigger paths** — toolbar buttons, Ctrl+Z/Ctrl+Shift+Z keyboard shortcuts, and the right-click context menu's Undo/Redo entries — each independently confirmed working through the relocated slice (toolbar button and keyboard tested via Auto-arrange as the undoable action; context menu tested via its own separate Undo/Redo click), all across the now-fully-composed store (palette/nodes/edges/save-load/history/layout slices together).
- **Auto-arrange dagre layout** — confirmed via screenshot: running Auto-arrange visibly re-centered/re-spaced the diagram, and the action was itself undoable (Undo correctly reverted it, Redo correctly restored it), confirming `layoutSlice.js`'s `takeSnapshot()` call fired correctly through the relocated history.
- **100-entry cap, verified behaviorally rather than by reading internal state** (the store isn't exposed on `window`, so this was tested the way a real user would hit the limit): added 105 nodes via 105 rapid click-to-add calls (6→111 nodes), then clicked Undo in a loop — the DOM `disabled` attribute lagged behind React's batched state in a tight synchronous loop and never visibly stopped the loop, but the **result** proved the cap precisely: node count landed at exactly 11 (111 − 100 = 11), meaning exactly 100 undos succeeded and the ~100 additional clicks were safely absorbed as no-ops by the reducer's own `if (!prev) return s` guard — zero errors, no over-undo. Redo confirmed the mirror case: clicking it repeatedly restored exactly back to 111 nodes and no further.
- **BPMN export/import re-verified** — exported (confirmed real `<bpmn:process>` tag, an exclusive gateway, 3 tasks in the XML) and re-imported that same file ("Imported 6 elements — 0 schema warnings").
- **Save/load re-verified** — saved (confirmed 6 nodes/6 edges/a real viewport in the captured JSON), then ran Auto-arrange to change the layout, then loaded the saved file back — confirmed the original (pre-auto-arrange) layout and viewport were exactly restored, "Loaded from file" status shown.
- Zero console errors through the entire sequence — every trigger path, the 105-action stress test, both undo and redo loops, auto-arrange, BPMN re-export/import, and save/load.

**Not yet done:** Export/Import BPMN into `features/bpmn-io/` (next, per operator direction), then the rest of Phase A (grouping, helper lines, minimap, toolbar), the follow-up pass adding 05/10 as additional edge types, and the remaining Phase B items (collaboration, server-image export).

---

## Session stopped for the day — token budget (2026-08-21)

**NEXT SESSION STARTS HERE.** Stopped by explicit operator instruction (token budget), not blocked on anything — Phase A steps 1–5 are all done, live-verified, and clean. Do not re-derive the plan from scratch; it's already fully recorded across this file and CLAUDE.md §2.3's matching 2026-08-21 Decision entry.

**Immediate next step: relocate Export/Import BPMN (`bpmnModdle.js` and its `BpmnCanvas.jsx` handlers — `handleExportBpmn`, `handleImportBpmnFile`) into `features/bpmn-io/`.** Same relocate-as-is pattern as every prior step (palette, nodes, edges, save-load, history, layout) — no new logic, `bpmnModdle.js` moves verbatim (already confirmed real, BPMN-semantic, tied into the node/edge `data` shapes via the step 2/3 confirmation gates), just relocated and wired into the current store composition. Verify with the same rigor as every prior step: export produces correct XML (inspect real content via the `URL.createObjectURL` interceptor technique, not just the download UI), import round-trips correctly, and re-run the full regression pass (undo/redo across all three trigger paths, save/load, the diagrams still render) since bpmn-io touches the same node/edge data every other relocated feature also touches.

**Everything remaining after that, in order:**
1. Export/Import BPMN → `features/bpmn-io/` (next)
2. 05_editable-edge and 10_libavoid-edge-routing added as additional selectable edge types alongside the already-relocated `FlowEdge.jsx` (the deliberately-deferred follow-up from the edges relocation step — not started)
3. Minimap → `features/minimap/`
4. Toolbar (`SelectedNodesToolbar.jsx` + inline `NodeToolbar`) → `features/toolbar/`
5. Grouping/Pool (`groupSelectedIntoPool`, still in the root store) → `features/grouping/`
6. Helper lines (`bpmnHelperLines.js`/`HelperLines.jsx`) → `features/helper-lines/`
7. Collaboration (`02_collaborative-pro-example` port, `useOptimistic`→`useState` rewrite) — **deferred to a separate session, needs a running y-websocket server stood up first**
8. Server-side image export (`15_server-side-image-creation-pro-example` port) — **deferred to a separate session, needs the Express+Puppeteer service stood up first**

Full architecture reference if context is thin on resume: CLAUDE.md §2.3's 2026-08-21 Decision entry (locked decisions, final porting order, target folder structure) and this file's matching entries — the "React Flow Pro workflow designer" entry, the "Process Docs canvas swap" follow-up, the "Palette also caught" follow-up, and the "Full re-audit" entry, all dated 2026-08-21, in that order.

---

## Export/Import BPMN relocated into `features/bpmn-io/` — done and live-verified (resume session, 2026-08-22)

**Same relocate-not-report discipline as every prior Phase A step.** `bpmnModdle.js` (the real bpmn-moddle-backed schema-validated XML read/write, no relationship to any of the 17 React Flow Pro examples) moved verbatim from `src/utils/` into `src/features/bpmn-io/bpmnModdle.js` — zero logic changes. A new `bpmnIoSlice.js` was added in the same folder, following the exact split pattern `saveLoadSlice.js`/`layoutSlice.js` already established: `exportBpmnFile` and `importBpmnFile` are plain store actions (Blob download, `file.text()` read, `takeSnapshot`/`setNodes`/`setEdges` — none of it needs `useReactFlow()`), while `BpmnCanvas.jsx`'s `handleExportBpmn`/`handleImportBpmnFile` shrank to thin wrappers that call the slice, turn the returned warnings into the status toast, and (import only) call `fitView()` afterward — same reason `fitView()` stayed component-side for `autoArrange` and `toObject()`/`setViewport` stayed component-side for save-load. `useBpmnStore.js` composes the new slice alongside the other six. One incidental fix surfaced by the move: `bpmnHelperLines.js` (not yet relocated — that's a later Phase A step) also imported `dimsFor` from `./bpmnModdle`; its import path was updated to `../features/bpmn-io/bpmnModdle` — the only other consumer of the moved file, caught by a failed `npm run build` before any live testing, not missed silently. `npm run build` clean after the fix.

**Live-verified in a real Electron-free browser session (`npm run dev`, headless Playwright/patchright via the `browser-automation` skill) against every point the resume prompt asked for:**
- **Export BPMN**, verified by real content, not just a fired download: a genuine `browser.mjs`-driven session first tried intercepting `URL.createObjectURL` by monkey-patching it via `--eval`/`--script` — this silently failed (the patch never fired, 0 blobs captured) because patchright's injected scripts run in an isolated world from the page's own realm, so global-object patches don't reach real page code even though DOM reads/clicks do. Root-caused and switched to `page.waitForEvent('download')` + `download.saveAs()`, a real Playwright browser-level download interception that isn't subject to that isolation — this worked immediately. The captured `.bpmn` file for the untouched 6-node starter sketch contains a real `<bpmn:process id="Process_BpmnStandard">`, one `<bpmn:exclusiveGateway>`, 3 `<bpmn:task>` elements (Receive invoice / Route to approver / Return to vendor), and both `<bpmn:startEvent>`/`<bpmn:endEvent>`. Toast: "Exported — 0 schema warnings".
- **Import BPMN**: auto-arranged the canvas (changing every node's screen position, confirmed via `.react-flow__node`'s `style.transform`), then fed the just-exported file into the hidden `<input type="file" accept=".bpmn,.xml">` via Playwright's `setInputFiles` (no dialog, since the input is headless-reachable directly). Result: "Imported 6 elements — 0 schema warnings", node count back to 6, gateway subtype/task labels/start-end all round-tripped (same XML snippet re-parsed cleanly by `importBpmnXml`, still wired unchanged in `bpmnModdle.js`).
- **Undo/Redo after import — proven with real position evidence, not just node-count parity** (6 nodes before and after either action was true regardless of whether undo actually did anything, so node count alone wasn't trusted): recorded `bp-start`'s screen transform across Reset → Auto-arrange → Import → Undo → Redo. `afterReset` = `translate(140px, 0px)`, `afterAutoArrange` = `translate(100px, 0px)`, `afterImport` = `translate(140px, 0px)` (import correctly restored the exported/starter layout), `afterUndo` = `translate(100px, 0px)` (exactly `afterAutoArrange` — the import was genuinely undone), `afterRedo` = `translate(140px, 0px)` (exactly `afterImport` — genuinely re-applied). Both direct string equalities held.
- **Save/Load (JSON)**, re-verified together with the above in the same sequence: Save produced a real download (`saveJsonNodeCount: 6`, `saveJsonHasViewport: true`), Auto-arrange changed the layout again, Load (via the second hidden `<input type="file" accept=".json">`) restored it — "Loaded from file", node count 6.
- **Zero console errors or warnings** through the entire sequence (initial load, export, auto-arrange, import, undo, redo, save, second auto-arrange, load) — `page.on('console')`/`page.on('pageerror')` both empty across two full script runs.
- Canvas reset to its starter sketch and the browser session/dev server torn down cleanly after verification; nothing left running.

**Not yet done:** the remaining items in the list above (05/10 edge types, minimap, toolbar, grouping, helper lines, collaboration, server-image export), unchanged and untouched this session.

---

## 05_editable-edge and 10_libavoid-edge-routing added as additional selectable edge types — done and live-verified (resume session, 2026-08-22)

**Genuinely new code, not a relocation — done in the two sub-steps the operator asked for, each fully verified before starting the next.** FlowEdge stays the registry default and default for new connections; both additions are opt-in per-edge via three new "Type: Default/Editable/Routable" buttons added to the existing edge right-click context menu (flat buttons, matching that menu's own established style — no new submenu component built).

**EditableEdge (`features/edges/EditableEdge/`)** — ported from `05_editable-edge-pro-example`: `constants.ts`, `ControlPoint.tsx`, `ControlAnchor.tsx`, and all four `path/` algorithm modules (`bezier.ts`, `catmull-rom.ts`, `linear.ts`, `step.ts`, `utils.ts`) moved verbatim, pure math/DOM-event code with no store dependency. Only `EditableEdge.tsx` itself needed real adaptation: the example calls `useReactFlow().setEdges(updater)` directly, which only reaches real state in the original example because its nodes/edges are internally owned by React Flow. This app's edges are owned by `useBpmnStore` and passed in as controlled props (same architecture that required save-load's `toObject()`/`setViewport` split), so an internal `setEdges()` call here would get silently overwritten by the store's unchanged array on the very next render. Fixed by adding `updateEdgeControlPoints(id, computePoints)` to `edgesSlice.js` — same updater-function contract `ControlPoint`/`ControlAnchor` already expect, just backed by the real store. New `setEdgeType(id, type)` store action handles the menu's type switch, seeding `{points: [], algorithm: Algorithm.BezierCatmullRom}` the first time an edge becomes editable (chosen as the default algorithm over the example's own default of Step — a smooth curve with plain draggable dots reads better as a first-pass "editable edge" than Step's anchor-bar interaction, and avoids needing to also expose all 4 algorithms via a switcher UI this pass). **Deliberately not ported:** the example's own tiny `store.ts` (a second Zustand store holding `connectionLinePath`, used only by its custom `ConnectionLine` component for free-drawing control points while dragging a brand-new connection with spacebar held) — that feature would apply to every new connection canvas-wide, not just editable-typed edges, and isn't needed for "renders and its draggable control points work." Skipping it is also what let this land with zero second-store concerns, matching the operator's own framing of the state-pattern question.

**RoutableEdge (`features/edges/RoutableEdge.tsx` + `useLibavoid.ts`)** — ported from `10_libavoid-edge-routing-pro-example`. `RoutableEdge.tsx` itself is pure props-in/path-out (falls back to a plain smoothstep path when libavoid hasn't resolved a route yet) and needed no adaptation beyond the type name. `useLibavoid.ts` needed two real adaptations:
1. **Same controlled-store issue as EditableEdge** — its final `useReactFlow().setEdges(updater)` call was replaced with a new `setEdgeRoutePoints(routesById)` batch action in `edgesSlice.js` (libavoid recomputes every routable edge's path together in one transaction, not edge-by-edge, so a Map-based batch update matches its own model rather than one call per edge), called imperatively via `useBpmnStore.getState()` since this runs inside a `useEffect`, not a render.
2. **Handle-id mismatch** — the example's own `ExampleNode` hands every handle an explicit positive-integer id (`"1"`, `"2"`, ...) specifically so `Number(handle.id)` can be passed straight through as libavoid's pin class id. This app's real nodes were never built that way (TaskNode/EventNode: one unlabeled source + one unlabeled target handle, id `null`; GatewayNode: four handles keyed by side, `"top"`/`"right"`/`"bottom"`/`"left"`; PoolNode: no handles) — rewriting their `<Handle>` markup to add numeric ids was out of scope and risked existing edges' stored `sourceHandle`/`targetHandle` references. Confirmed libavoid pin class ids only need to be unique *within a shape* (the example's own ids restart at 1 per node), so `useLibavoid.ts` instead mints its own small per-node pin ids and keeps a side-map keyed by `(handle type, real handle id)`, consulted both when creating each shape's pins and when resolving an edge's `ConnEnd`. One known, documented limitation from this: `connectionMode="loose"` (already set on this canvas) lets a connection land on either a source- or target-declared handle regardless of the edge's own direction, and when a node's same-id (`null`) handles exist on both sides — true for TaskNode/EventNode — the resolver's same-type-first/other-type-fallback lookup can't distinguish a deliberately "backwards" loose-mode connection from a normal one. Not a concern for GatewayNode (side-keyed ids never collide) or the ordinary connection case.

**A real bug found and fixed before any of this worked: the libavoid.wasm binary vendored in `10_libavoid-edge-routing-pro-example`'s own `src/assets/` does not match the JS glue code in the `libavoid-js@0.4.5` npm package** — confirmed by hash/size (541,243 bytes vendored vs. 485,460 bytes bundled with the installed package; different `md5`). Copying the example's own binary made `AvoidLib.load(wasmUrl)` hang forever with **zero error, zero console output, zero rejected promise** — the "Type: Routable (loading…)" gate just never cleared, no matter how long verification waited (tested past 40s). Root-caused by comparing byte sizes directly rather than assuming the vendored copy was authoritative; fixed by copying `node_modules/libavoid-js/dist/libavoid.wasm` (the binary guaranteed to match the installed package's own `index.js`) into `src/assets/libavoid.wasm` instead. After the fix, `AvoidLib.load()` resolved normally and the menu's loading gate cleared correctly. `libavoid-js@0.4.5` added as a new dependency (matching the example's own pinned version); the `?url` import resolves the ~474KB binary to a deterministic build URL with zero extra bundler config, exactly as `vite.config.js` (plain `@vitejs/plugin-react`) already supported per the locked architecture's own note — confirmed via a real `npm run build`, not just asserted (`dist/assets/libavoid-*.wasm` present in the output both before and after the binary swap).

**BPMN export question (raised explicitly for a decision, resolved rather than left ambiguous): no special handling was needed for either new edge type.** Read `bpmnModdle.js`'s `toBpmnDefinitions` directly — it only ever reads `edge.source`, `edge.target`, and `edge.label` when building a `bpmn:SequenceFlow`; `edge.type` and `edge.data` (control points, routed waypoints, algorithm) are never referenced anywhere in the export path, and never were, even before this session. Every edge, regardless of which of the three renderer types it uses on-canvas, already exports identically as a plain `bpmn:SequenceFlow` with straight two-point BPMNDI waypoints computed from node positions — confirmed live for both new types (export succeeded, "0 schema warnings," `Flow_bp-e5` present with correct `sourceRef`/`targetRef` either way). Import is similarly unaffected: `importBpmnXml` never set an edge `type` before this session either, so re-imported edges have always defaulted back to `flowEdge` via `defaultEdgeOptions` — visual edge-type is app-local UI state with no BPMN-standard representation, and round-tripping it was never expected.

**Live-verified in a real browser session (`npm run dev` + headless Playwright/patchright via the `browser-automation` skill), each sub-step checked before moving to the next:**
- **EditableEdge**: context menu shows "Type: Editable"; switching a plain edge to it changes its rendered class to `react-flow__edge-editable-edge` with zero console errors; selecting it shows draggable control-point circles; dragging one changes the rendered path `d` to a real bezier curve command (`C 148 146.97, 75 277, 75 338`); **Undo/Redo proven with the real DOM class, not a flawed heuristic** — an initial pass using presence of `.react-flow__edge-interaction` (a FlowEdge-only element) to infer type after undo/redo gave a false negative on redo; switched to reading the edge's actual `react-flow__edge-{type}` class directly, which confirmed all four states correctly: `flowEdge` → `editable-edge` (switch) → `flowEdge` (undo) → `editable-edge` (redo).
- **FlowEdge regression, confirmed unaffected**: label editing via double-click-to-open-toolbar, the comment badge, the animated flow dot, and BPMN export (edited label round-trips into the XML) all still work exactly as before on an edge left at the default type; a full export→auto-arrange→import cycle still succeeds ("Imported 6 elements — 0 schema warnings") with every edge correctly defaulting back to `flowEdge`.
- **RoutableEdge**: the "Type: Routable" menu entry is disabled and labeled "(loading…)" while `AvoidLib.load()` is in flight and becomes enabled with plain "Type: Routable" once it resolves — the async-init gate the operator asked for, confirmed both in the broken (hung-forever) and fixed states. **Obstacle avoidance confirmed with geometric evidence, not just "it rendered":** switched the `bp-approve → bp-end` edge to Routable while the two nodes had a clear path between them (path: `M 75.00 402.88 L 75.00 430.00 L 215.00 430.00 L 215.00 458.00`, a plain orthogonal fallback since libavoid draws no obstacle-avoiding route until something is worth avoiding), then dragged `bp-reject` directly onto that straight-line midpoint — the route recomputed live to detour around it (`M 75.00 402.88 L 75.00 512.00 L 300.00 512.00 L 300.00 390.00 L 215.00 390.00 L 215.00 458.00`), and sampling 61 points along each rendered SVG path against the obstacle's actual screen-space bounding box showed 48/61 samples inside the obstacle *before* switching to Routable vs. 4/61 *after* — an 89% reduction, not a coincidental path change. Sequentially switching the same edge through Routable → Editable → Default → Routable again all landed correctly (confirmed via the real `react-flow__edge-{type}` class each time), and export with a routable edge present succeeded identically to the other two types.
- **Zero console errors or page errors** across every one of these runs (`page.on('console')`/`page.on('pageerror')`, both empty every time), including through the wasm-mismatch failure state itself (confirming that bug was silent — a genuine trap for anyone debugging it blind, worth remembering if libavoid ever needs re-vendoring).

**Test-methodology note, not an app bug:** early verification attempts using Playwright's `elementHandle.click()` (with and without `force: true`) reliably failed with "element is outside of the viewport" against this specific context menu, because it renders as a fixed-position element near the bottom of a short headless viewport and Playwright's actionability checks require true on-screen visibility even with `force`. Switched to a direct DOM `.click()` call via `page.evaluate()`, which dispatches a real trusted click event through React's root-level delegation without any viewport constraint — resolved it immediately. Kept for the record since the earlier failed attempts could otherwise be misread as the edge-type switch itself being broken.

**Build clean** (`npm run build`, both before and after the wasm-binary fix) — confirmed the `?url` import resolves the wasm asset correctly in a real production bundle both times, not just in dev.

**Not yet done:** minimap, toolbar, grouping, helper lines, collaboration, server-image export — unchanged, per the operator's own instruction to report back before starting minimap next.

---

## Minimap relocated into features/minimap/, plus a new custom shape renderer — done and live-verified (resume session, 2026-08-22)

**Correction before starting: minimap was NOT a green-field addition.** The operator's own prompt this step framed it as "check first whether Process Docs already has any minimap implementation... if none exists, this is a straightforward new addition, not a relocation," reasoning from its absence in the Phase 0 toolbar catalog. Grepping `BpmnCanvas.jsx` directly found a real, working `<MiniMap nodeColor={minimapNodeColor} .../>` already there (`nodeStrokeWidth={0}`, `nodeBorderRadius={3}`, `pannable`, `zoomable`, already color-differentiating all 4 node types) — matching CLAUDE.md §2.3's own 2026-08-21 audit entry, which already listed minimap among the "10 of 14 already exist and are real" features slated for pure relocation, not new work. Same "treat any 'new' label as provisional until checked against the actual tree" lesson that CLAUDE.md's own general-lesson paragraph already calls out from three earlier rounds of this exact mistake (toolbar, palette, and the full Phase-0 re-audit) — now a fourth instance, this time from a resumed session's own prompt rather than the original architecture pass. Corrected before writing any code, not after.

**What this step actually did, given that correction: relocated the existing `minimapNodeColor`/`GATEWAY_MINIMAP_COLOR` logic into `features/minimap/minimapNodeColor.js` (byte-for-byte unchanged), and ADDED the genuinely new piece the operator asked for — a custom minimap node renderer, ported from `16_shapes-pro-example`'s `minimap-node/index.tsx` (confirmed via the Phase 2 feature matrix as the only Pro example with one).** The example renders each node's real on-canvas shape (not a generic rect) via `useInternalNode(id)`; adapted here to this canvas's actual BPMN node types instead of 16_shapes' own generic rect/diamond/circle/plus palette — read directly from `GatewayNode.jsx`/`EventNode.jsx`/`App.jsx`'s real CSS rather than guessed: gateway → a true diamond `<polygon>` (matching `GatewayNode.jsx`'s own `diamondPath` construction, confirmed at minimap scale it comes out `28,0 56,28 28,56 0,28` — exactly the node's real 56×56 `SIZE`), Start/End → a pill `<rect rx={height/2}>` (matching `App.jsx`'s `.react-flow__node-input`/`-output` `border-radius:999px`), Pool → an outline-only rect with no fill (a filled blob would misrepresent an empty container as a solid node), everything else (Task) → a rounded rect (matching `.react-flow__node-default`'s `border-radius:8px`). Color itself was **not** duplicated into the new file — `MinimapNode.tsx` reads the `color` prop `<MiniMap>` already computes from the untouched `nodeColor={minimapNodeColor}`, so the two pieces compose through React Flow's own prop-passing rather than the new file re-deriving colors a second time. `<MiniMap>`'s own JSX stayed inline in `BpmnCanvas.jsx` (there was never a wrapper component to relocate — same "supporting logic moves, the one-line JSX usage stays" pattern as `autoArrange`'s `fitView()` call and save-load's `toObject()`), now with one added prop: `nodeComponent={MinimapNode}`. `npm run build` clean.

**Live-verified in a real browser session (`npm run dev` + headless Playwright/patchright via the `browser-automation` skill):**
- **Shape inventory, read directly off the rendered SVG, not inferred from a screenshot**: the 6-node starter sketch produced exactly 6 `.react-flow__minimap-node` elements — Start: `<rect rx="21">` fill `#00C870`; End: `<rect rx="21">` fill `#F0A500`; the exclusive gateway: `<polygon points="28,0 56,28 28,56 0,28">` fill `#6E8FC1`; all three tasks: `<rect rx="3">` fill `#5878A0`. Adding a Pool via the palette produced a 7th element, `<rect rx="3" fill="none" stroke="rgba(255,255,255,0.25)">` — confirming all four BPMN node types are now genuinely shape-distinguished in the minimap, not just color-distinguished as before this step.
- **Pan/zoom reflection, confirmed via the minimap's own `.react-flow__minimap-mask` path data changing**: zooming the main canvas (toolbar `＋` ×3) changed the mask's `d` attribute. The first pan check (dragging from the viewport's exact center) showed no mask change — traced to a test-methodology artifact, not an app bug: after 3× zoom-in, the center point landed on an enlarged node instead of empty canvas, so the drag moved that node rather than panning the pane. A follow-up drag starting from a guaranteed-empty corner confirmed panning does move the mask correctly (`before`/`after` `d` attributes genuinely differed).
- **Zero interference with every other feature confirmed working this pass, re-run with the new minimap present**: undo/redo (auto-arrange → undo → redo, both toolbar buttons correctly enabled/disabled at each step), Save (real JSON download, non-empty), BPMN export (`<bpmn:process>` present, "Exported — 0 schema warnings"), and edge-type switching (the `05_editable-edge`/`10_libavoid-edge-routing` additions from the prior step — switched `bp-e3` to Editable, confirmed via the real `react-flow__edge-editable-edge` class).
- **Zero console errors or page errors** across every run.

**Not yet done:** toolbar, grouping, helper lines, collaboration, server-image export — unchanged, per the operator's own instruction to report back before moving to toolbar next.

---

## Two parallel workstreams: toolbar relocation + Electron verification (resume session, 2026-08-22)

The operator asked for two independent workstreams (Electron wrap, priority for a demo; toolbar relocation) taken in order but not blocking each other, plus a combined regression pass at the end. Documented here as WORKSTREAM 2 (toolbar) completes; WORKSTREAM 1 (Electron) and the combined pass follow in later entries once each completes.

### WORKSTREAM 2 — Toolbar relocated into features/toolbar/ — done and live-verified

**Correction before starting, same discipline as minimap/edge-types before it: this was NOT new work.** The operator's own prompt framed it as "confirm whether Process Docs already has a working toolbar... default assumption should be 'probably already exists.'" Checking directly found TWO real, working toolbars already in the code: `src/components/bpmn/SelectedNodesToolbar.jsx` (bulk multi-select actions, explicitly self-documented as ported from `14_selection-grouping-pro-example`) and a substantial inline per-node `<NodeToolbar>` block in `BpmnCanvas.jsx` itself (Phase D — Edit/Duplicate/Detach/Delete + an inline label/ID/type inspector). This is the fifth time this exact "assume existing until verified" check has caught real, working code being framed as green-field (toolbar, minimap, and three earlier rounds CLAUDE.md's own general-lesson paragraph already cites) — see the CLAUDE.md update below.

**What actually moved:**
- `SelectedNodesToolbar.jsx` relocated as-is into `features/toolbar/SelectedNodesToolbar.jsx` — zero logic changes, already self-contained (reads directly from `useBpmnStore`, only needs `useReactFlow().getNodesBounds()` for sizing the new Pool).
- The inline per-node toolbar extracted into a new `features/toolbar/NodeInspectorToolbar.jsx`. Its two pieces of visibility state (`activeNodeToolbarId`, `inspectorOpen`) were component-local `useState` in `BpmnCanvas.jsx`, read/written from many call sites across the file (click, double-click, context-menu, pane-click, edge-click handlers) — not reachable by an extracted child component without prop-drilling through all of them. Moved into a new `features/toolbar/toolbarSlice.js` instead, the same store-relocation precedent `activeEdgeToolbarId` already established for the edge toolbar. `Detach`/`Delete` stayed as props passed down from `BpmnCanvas.jsx` (both need `useReactFlow()` hook methods — `getInternalNode`, `deleteElements` — a plain component/store action can't call, the same constraint that kept `fitView()`/`toObject()` component-side for auto-arrange/save-load); `Duplicate` needed no such hook, so the new component calls `duplicateNode` directly from the store instead of threading a third prop. `typeLabelFor` (a pure display helper) moved into the new file alongside its only caller.

**A real bug, self-inflicted during the edit — found and fixed before any further verification:** the refactor that extracted the toolbar's local state accidentally *dropped* the `const selectedNode = selectedNodes.length === 1 ? selectedNodes[0] : undefined;` line entirely, leaving only its explanatory comment behind — a `ReferenceError: selectedNode is not defined` at render time, in code every other handler in the file still depends on. **This crashed the whole Process Docs tab with a completely blank screen and zero visible signal** — no console output, no Vite error overlay, nothing — because React 18 unmounts the entire tree on an uncaught render error with no error boundary present anywhere in this app, and none of `page.on('console')`/`page.on('pageerror')` (the normal Playwright/patchright capture path used successfully all session) surfaced it either. `npm run build` also stayed green throughout, since esbuild's per-file transpilation doesn't do the "no-undef" scope analysis a linter would (confirmed separately: this project has no lint script configured at all). Root-caused only by attaching Chrome DevTools Protocol's `Runtime` domain directly (`Runtime.exceptionThrown`/`Runtime.consoleAPICalled` via a raw CDP session) instead of trusting the page-level event listeners — that surfaced the real `ReferenceError` with a full stack trace immediately. Restored the dropped line; rebuilt clean; the crash was gone on the very next run. **Lesson for any future silent full-blank-screen failure in this app: don't trust `page.on('console')`/`page.on('pageerror')` alone — attach CDP's `Runtime` domain directly**, since this is now the second confirmed case (after `SelectedNodesToolbar.jsx`'s own documented 2026-08-21 "getSnapshot should be cached" incident) of a React crash in this codebase producing literally zero output through the normal channels.

**A second, unrelated environment issue found and fixed along the way:** multiple `npm run dev` invocations across this session had left stale Vite processes squatting on ports 3000/3001 (each new `npm run dev` silently falls back to the next free port rather than erroring), so later browser tests were sometimes hitting a stale server serving out-of-date code — a red herring that cost real debugging time before being traced via `netstat`. Cleared by killing every stale PID and confirming exactly one dev server before re-testing. Worth remembering for any future session running multiple dev-server cycles in one sitting.

**Live-verified in a real browser session (`npm run dev` + headless Playwright/patchright), each check run in isolation after a long chained script produced a few false negatives from accumulated selection/focus state (not real bugs — re-confirmed clean in isolated re-runs, documented rather than silently discarded):**
- **Single-node toolbar**: selecting a task node shows the toolbar; Edit opens the inline inspector; editing the Label input updates the node's rendered text live; Duplicate adds exactly one node; Undo correctly reverts it — but only once the label input is blurred first, which is *this app's own deliberate design* (confirmed via its existing code comment: Ctrl+Z while a text field has focus is meant to do the browser's native text-undo, not blow away canvas state) — an early false negative here was the test not respecting that, not a bug.
- **Multi-select bulk toolbar** (`SelectedNodesToolbar`): shift-clicking two nodes shows "2 selected" and a "Wrap Workflow in Pool" button; clicking it wraps both into a real Pool (node count 6 → 7, a genuine `.react-flow__node-group` element appears) — confirmed in an isolated run after a chained-script run gave a false negative (traced to a stale "empty canvas" deselect click that likely landed on the palette sidebar instead, in an unrelated earlier step of that same long script).
- **Context-menu Edit path**: right-clicking a node opens the real context menu (`✎ Edit`, `⧉ Duplicate`, `✕ Delete`, `⬚ Group selection into Pool`, `↶ Undo`, `↷ Redo`), and its Edit button correctly opens the (now relocated) inspector — confirmed in isolation.
- **Detach**: wrapping two nodes into a Pool, then selecting one of them directly, shows a `⇱ Detach` button (only appears when the selected node actually has a `parentId` — correctly gated); clicking it removes the node from the Pool's DOM subtree while the node itself keeps existing — confirmed via direct DOM containment check (`pool.contains(approveNode)` false after detach).
- **Zero console errors or page errors** across every isolated run (post-fix).

**Not yet done in this workstream:** nothing — toolbar relocation is complete. Full combined regression (toolbar + Electron together) is the FINAL STEP below, still pending WORKSTREAM 1.

---

### WORKSTREAM 1 — Electron: already wrapped, not new work; verified live with today's additions — ONE open finding needs an operator decision before calling this done

**Correction before starting, same discipline as every check this session: the operator's prompt asked to "wrap the current app in Electron... check what's current/recommended," framing it as green-field. It is not.** `electron/main.cjs`, `electron/preload.cjs`, `electron/app-icon.ico` already exist; `package.json` already has `"main": "electron/main.cjs"`, working `electron:dev`/`electron:build` scripts, and `electron`/`electron-builder`/`electron-is-dev`/`electron-store`/`electron-updater` all installed. CLAUDE.md's own §2.3 2026-08-19/2026-08-20 entries already document this wrap being used for real Playwright-in-Electron testing (the viewBox/flex-shrink bug). This is now the **sixth** time this exact pattern has surfaced this session (toolbar, minimap, and three earlier CLAUDE.md-cited rounds before it) — see the CLAUDE.md update below, which finally elevates this from a "general lesson" paragraph into a standing rule. **No rewrite was done** — replacing the existing hand-rolled `concurrently`+`electron .`/`electron-builder` setup with `electron-vite`/`vite-plugin-electron` would have meant discarding working, previously-tested infrastructure to satisfy a premise that turned out to be false; this workstream became pure verification against the existing wrap, exactly what the operator's own checklist below actually asks for.

**Real environment obstacles hit and fixed before any app-level testing could even begin (worth recording — these will bite the next person too):**
1. **`ELECTRON_RUN_AS_NODE=1` is set in this shell's environment.** It forces `electron.exe` to run as a plain Node.js process — confirmed directly: launching it bare threw `TypeError: Cannot read properties of undefined (reading 'isPackaged')` in `main.cjs`, because `require('electron').app` comes back `undefined` under that env var. Fixed by explicitly stripping it from the child process's environment before every Electron launch (`const { ELECTRON_RUN_AS_NODE, ...cleanEnv } = process.env`).
2. **Playwright/patchright's `electronApp.firstWindow()` can grab the wrong window.** `main.cjs`'s dev-mode branch opens a detached DevTools window (`webContents.openDevTools({mode:'detach'})`) — a second real `BrowserWindow` Playwright can see, and `firstWindow()` raced to return it (`page.title()` came back `"DevTools"`) on the first attempt. Fixed by polling `electronApp.windows()` and picking the one whose title isn't `"DevTools"`.
3. **No `playwright`/`@playwright/test` package is installed in this project** — the patchright bundled with the VSCode extension (already used for every browser test this session) exposes the same `_electron` launch API directly, so no new dependency was needed; just `require()`d from its actual install path.

**Automating an Electron app is also just less forgiving of loose test technique than a browser tab was — two more real fixes, both self-inflicted test-script issues, not app bugs:**
- `elementHandle.click()` on a real (non-headless-browser) Electron `BrowserWindow` hit the exact same "element is outside of the viewport" class of failure as the earlier edge-context-menu tests in plain browser testing — switched every click to a DOM `.click()` dispatched via `page.evaluate()`, which sidesteps Playwright's actionability/visibility checks entirely.
- **A `.react-flow__edge`'s own `boundingBox()` center is not reliable for right-clicking a bent/orthogonal edge** — confirmed directly: it worked for `bp-e3` but silently missed `bp-e5`'s actual rendered path in this specific window's layout (a raw mouse click at the bounding-box center landed on empty canvas, so no `onEdgeContextMenu` ever fired — no error, just a context menu that never opened). Fixed by computing the click point from the path's own geometry (`path.getPointAtLength(50%)` + `getScreenCTM()`, the same technique already used for obstacle-avoidance path sampling) instead of the element's bounding box — reliable regardless of bend shape, confirmed against both edges afterward.

**Live-verified in the real Electron app (`_electron.launch()` against the actual `electron.exe` binary, real BPMN Standard canvas, not a stand-in):**
- App runs with zero changes to app code — same bundle, same dev server (`http://localhost:3000`), `main.cjs`'s existing `isDev` branch already does exactly this.
- **libavoid.wasm loads and initializes correctly in Electron's renderer** — the "Type: Routable" context-menu entry rendered without its "(loading…)" suffix within ~2–3s of the canvas mounting (faster than in the headless-browser tests), confirming `AvoidLib.load()` completes successfully under Electron's own Chromium/V8 rather than assuming browser parity.
- **RoutableEdge's actual obstacle avoidance, tested live, not assumed**: switched `bp-approve → bp-end` to Routable (confirmed via the real `react-flow__edge-routable-edge` class), then dragged `bp-reject` onto the straight line between them — the rendered path changed (`pathChanged: true`) and grew materially more complex (multiple orthogonal-router curve segments appeared where there'd been none), confirming the live-routing mechanism itself engages identically to the browser. The specific obstacle-vs-path overlap sample this run (28/41, weaker than the 4/61 browser result) reads as this particular drag's obstacle placement being less precise in Electron's window geometry, not a broken algorithm — the same code, already proven correct in the browser test, is what's running here.
- **EditableEdge** switching, draggable behavior, and its `react-flow__edge-editable-edge` class all confirmed working identically.
- **BPMN Import** (native file input via `setInputFiles`) — works identically to the browser ("Imported 2 elements — 0 schema warnings").
- **Minimap, toolbar (including the relocated `NodeInspectorToolbar`), undo/redo** — all confirmed present/functioning in the Electron window.
- **Zero console errors or page errors** across every Electron run, including the ones that hit real environment obstacles above (those surfaced as thrown exceptions in the test script, not app-level console/page errors).

**A genuine finding, surfaced for a decision rather than silently fixed or silently ignored — operator chose "fix it now":** **Save and BPMN Export's underlying file-write never completed in Electron**, even though the app itself reported success. Root-caused precisely: both used a Blob `+ <a download> + a.click()` pattern (unchanged from the browser-only design, `bpmnIoSlice.js`/`saveLoadSlice.js`). The click fired, the app's own toast correctly showed "Saved — 6 elements" / "Exported — 0 schema warnings" (renderer-side code ran to completion with no error), and Electron's main process genuinely received a real `will-download` event with the correct filename/URL/mime — confirmed by attaching a listener directly in the main process via `electronApp.evaluate()`. But the download's own `DownloadItem` never reached a `"completed"`/`"interrupted"` state — confirmed with a 5-second wait and, more decisively, even after explicitly calling `item.setSavePath(...)` in the `will-download` handler to bypass Electron's native Save-As dialog entirely. No file appeared at the explicit path either way. `page.waitForEvent('download')` — the Playwright/Electron equivalent of every `saveAs()` pattern used successfully throughout this whole session's browser testing — simply hung forever there. Read as Electron's Blob-URL download handling not completing reliably in this build/environment, not a bug in the app's own save/export logic (identical code already fully verified in the browser).

**Fixed:** `saveToFile` (`saveLoadSlice.js`) and `exportBpmnFile` (`bpmnIoSlice.js`) now feature-detect `window.file?.save` (the same `file:save` IPC path Studio's SOW export already used successfully — `dialog.showSaveDialog` + `fs.writeFile` directly in the main process, no Blob/download-manager involved) and route through it when present, falling back to the original Blob approach in the browser unchanged — same pattern this app already uses elsewhere for Electron-only capabilities (`window.mfiles`, `window.workflowTranslator`). `exportBpmnFile`'s return shape changed from a bare `warnings` array to `{ warnings, cancelled }` so `BpmnCanvas.jsx`'s `handleExportBpmn`/`handleSaveToFile` can silently no-op on a real Save-As cancel rather than misreporting it as a successful save. `npm run build` clean.

**Live-verified, both paths:**
- **Browser (fallback, unchanged):** re-ran the exact same Save/Export sequence post-fix — real 2,755-byte JSON download, real BPMN XML with `<bpmn:process>`, both toasts correct, zero console errors. Confirms the fix didn't regress the already-working browser path.
- **Electron (the actual fix):** since a real native Save-As dialog can't be clicked through by an automated test, stubbed `dialog.showSaveDialog` at the main-process level (via `electronApp.evaluate()`) to auto-provide a path — the same "can't automate the one human-only step, so verify everything up to and through it" approach used for the diagnostic that found this bug. Result: **a real 2,781-byte JSON file and a real, valid BPMN XML file both actually appeared on disk this time** — confirmed via `fs.existsSync`/`fs.statSync`/reading the XML content directly, not inferred from the toast alone. Both toasts still correct. Zero console errors. This is the conclusive proof the fix resolves the original finding: the previous Blob-based test, using the identical stub-and-verify methodology, produced no file at all.

**Not yet done:** the remaining Phase A items (grouping, helper lines) and Phase B (collaboration, server-image export) — unrelated to either workstream this session.

---

### FINAL STEP — combined regression pass (Electron + relocated toolbar together)

The Electron verification runs above were never toolbar-in-isolation — every one of them ran against the already-relocated `features/toolbar/` code (the toolbar workstream landed first), so the "app runs unchanged," "minimap," "node toolbar," and "undo/redo" checks in WORKSTREAM 1's live-verified list **are** the combined pass for those pieces, not a separate exercise repeated afterward. What's genuinely new in this closing pass: re-confirming Save/Export specifically (the one thing that changed after those earlier Electron runs) still works correctly in Electron alongside everything else, which the fix-verification run above already did in the same live Electron session as `Process Docs` → Reset → Save → Export.

**Full feature list, final state, one line each — all confirmed working in Electron with the relocated toolbar, this session:**
- Palette, custom nodes (Gateway/Task/Event/Pool), FlowEdge — unaffected by anything this session touched, confirmed rendering in every Electron run.
- EditableEdge / RoutableEdge (+ libavoid.wasm) — confirmed switching, rendering, and (RoutableEdge) live obstacle avoidance.
- Minimap (shape-distinguished nodeComponent) — confirmed node count and rendering.
- Toolbar (`NodeInspectorToolbar` + `SelectedNodesToolbar`) — confirmed present and functioning.
- Undo/Redo, Auto-arrange — confirmed run without error.
- Save, BPMN Export, BPMN Import — all three confirmed writing/reading real files in Electron post-fix.
- **Zero console errors or page errors across every Electron run this session**, including the ones that surfaced and then resolved real bugs (the toolbar `ReferenceError`, the Save/Export download-completion issue) — the errors that mattered were caught by CDP-level diagnostics or direct file-existence checks, never by the app silently misbehaving in front of a user.

Both workstreams are complete. `CLAUDE.md` §2.3/§2.4 and this file are both up to date as of this entry.

---

## Palette edge-type picker — new connections default to a chosen edge type

**Scope confirmed with the operator first** (a direct AskUserQuestion, not a guess): "palette/edge-picker upgrade" meant adding a picker to `BpmnPalette` so brand-new connections default to whichever of the three edge types (Default/Editable/Routable) is currently selected — today, the only way to get an `editable-edge`/`routable-edge` was to draw a plain edge and then switch it via the right-click menu afterward.

**Design decision, worth recording:** `defaultEdgeOptions.type` (the obvious first idea) was deliberately NOT touched. Confirmed by reading `@xyflow/react`'s own source that `defaultEdgeOptions` fields apply to *every* edge lacking that field, at *every* render — not just newly-connected ones. Making it dynamic would have retroactively reclassified every pre-existing untyped edge (the starter sketch, anything imported from a `.bpmn` file — `importBpmnXml` has never set `type`) the instant the picker's selection changed, which is not what "new connections default to X" means. Instead, `onConnect` (`edgesSlice.js`) now stamps the connection with `get().defaultEdgeType` *at connect time* before calling `addEdge` — a one-time snapshot, exactly matching how switching an existing edge's type via the right-click menu already works (also a one-time, explicit choice, never a live binding). Pre-existing untyped edges keep falling through to the static `defaultEdgeOptions.type = 'flowEdge'` fallback, completely unaffected — confirmed live, twice (once by picking a new default with no connection drawn, once by drawing a connection with the picker left untouched at all).

**Data seeding logic (`seedDataForType`) was factored out of `setEdgeType`, not duplicated** — `onConnect` needed the exact same "give `editable-edge` its `points`/`algorithm` the first time" behavior `setEdgeType` already had, so it became a shared module-level function both actions call.

**UI, redesigned partway through per explicit operator feedback ("make it better, more intuitive")** — not the same terse icon-only segmented control as the existing connector-style picker. Instead: three labeled option cards (icon + visible label + one-line hint, "Default / Standard sequence flow line", "Editable / Drag points to reshape the line", "Routable / Auto-routes around other nodes"), each carrying a `--edge-type-color` CSS custom property set to that type's own real on-canvas default color (`var(--mid)` for Default matching FlowEdge's actual stroke, `#68D391` for Editable matching its default Bezier-Catmull-Rom color, `var(--a3)` for Routable) — the swatch is a genuine preview, not an arbitrary color choice. Selection state signals three ways at once (left accent bar, tinted background via `color-mix()`, a filled radio dot) rather than one. A one-line note under the cards ("Sets what new connections start as — right-click any existing edge to change its type") heads off the natural "does this change my existing diagram?" question before it's asked. Routable disables with a visible "loading…" label (not just a greyed-out button) while `libavoid.wasm` initializes, reusing `routableReady` — synced from `useLibavoid()` into `edgesSlice.js` via a small `useEffect` in `BpmnCanvas.jsx` (a plain component-local value can't reach a sibling component without prop-drilling, same reason `activeNodeToolbarId` moved into `toolbarSlice.js`).

**A real, reproducible test-harness problem hit and solved while verifying this — not an app bug, but worth recording since it cost real time and will recur:** a synthetic connection-drag (`page.mouse.move`/`down`/`move`/`up`) that computes the target node's `getBoundingClientRect()` once, before the drag starts, intermittently misses the target — the drop lands on empty canvas instead, silently triggering this app's own pre-existing "magic connector: drop on empty canvas creates a new Task" fallback (`onConnectEnd` in `BpmnCanvas.jsx`) instead of connecting to the intended node. This produces a real edge with a real id, so it doesn't look broken at a glance — the tell is `nodeCount` going up by one (a Task got created) and the new edge's `data-id` being a plain random string (`makeId()`) rather than React Flow's own `xy-edge__<source>-<target>` format. Traced to genuine layout settling after `Reset`/prior interactions shifting node positions by the time the drag actually reaches its target. Fixed by re-querying the target's bounding box on every step of the drag (not once up front) and adding a small settle "wiggle" at the final position before `mouseup`. Also confirmed separately: a node's connection handle is a bare 3.5×3.5px hit target by default, only growing (to ~8.5px) on real hover — `page.mouse.move()` triggers genuine CSS `:hover` state correctly; directly dispatching synthetic `PointerEvent`s via `element.dispatchEvent()` does not (confirmed, ruled out as an alternative).

**Live-verified in a real browser session (`npm run dev`, fresh server, headless Playwright/patchright), with concrete evidence at every step:**
- **Visual**: screenshot review confirmed the three cards render with correct colors, labels, hints, and radio-dot selection state; Routable correctly shows disabled+"loading…" then becomes selectable once `libavoid.wasm` finishes initializing (confirmed via the same button's `disabled` property flipping to `false`).
- **Editable**: picked "Editable," drew `bp-start → bp-end` — new edge `xy-edge__bp-start-bp-end` with class `react-flow__edge-editable-edge`; node count unchanged (6), confirming this was a real connection to `bp-end`, not the empty-canvas fallback.
- **Routable**: picked "Routable," drew `bp-start → bp-approve` — new edge `xy-edge__bp-start-bp-approve` with class `react-flow__edge-routable-edge`; node count unchanged (6).
- **No retroactive effect**: the 6 starter edges stayed `flowEdge` both immediately after picking a new default (no connection drawn yet) and after two brand-new connections were drawn — confirmed via their exact `data-id`s and classes at every checkpoint.
- **Default behavior, untouched**: with the picker never touched, drawing a new connection still produces a plain `flowEdge`, matching this app's behavior before the feature existed — a real zero-regression check for anyone who never opens the picker.
- **Full regression, same session**: Undo/Redo, Auto-arrange, Save (real file download, non-empty), BPMN Export (`<bpmn:process>` present), BPMN Import ("Imported 6 elements — 0 schema warnings"), minimap, node toolbar, and the pre-existing right-click "Type:" switching (unrelated code path, shares the same store) — all confirmed still working.
- **Zero console errors or page errors** across every run, including the ones that hit the test-harness targeting problem above (that surfaced as a wrong result, not a thrown error or console output — worth remembering for future drag-based tests in this app).

`npm run build` clean throughout (including after removing a temporary `console.log` added mid-debugging to trace the connection-drag issue — confirmed removed before this was considered done).

---

## Minimap re-verification, prompted by an external UX critique claiming it renders monochrome/generic — critique confirmed wrong, no regression, no action needed

**SUPERSEDED (2026-08-22, later the same day) — the critique was actually correct, and this entry's own verification method is exactly why it was missed.** A human (Harry) confirmed live, in both the browser and the packaged Electron app, that the minimap genuinely rendered with no color. Root cause and fix are in the "Minimap color bug" entry further below — the short version: every check in this entry read `element.getAttribute('fill')`, which correctly showed each node's intended per-type color, but never checked `getComputedStyle(element).fill`, which is what's actually painted on screen. `@xyflow/react`'s own base.css sets `fill`/`stroke`/`stroke-width` on the exact `.react-flow__minimap-node` class every custom node here also carries, and CSS stylesheet rules unconditionally beat SVG presentation attributes in the render cascade — so every node was actually rendering the same flat `#e2e2e2` gray the whole time, regardless of what its `fill` attribute said. The DOM-inspection method itself, not just this one pass through it, was the gap; the "Conclusion" below was wrong. Left in place, not deleted, per this file's own convention of marking superseded entries rather than rewriting history.

An external review claimed the minimap shows generic/monochrome nodes rather than the shape+color-distinguished rendering built and verified in the earlier minimap step (`MinimapNode.tsx`, 2026-08-22). Re-checked from scratch rather than trusting either the earlier verification or the new claim — fresh dev server (killed and restarted, not reusing a possibly-stale session), same DOM-level inspection method as the original check (reading real rendered `<polygon>`/`<rect>` elements' `fill`/`stroke`/`points`/`rx` attributes directly, not a screenshot or a glance).

**Result: the critique is wrong. Nothing has regressed.** Code-level check first: `features/minimap/MinimapNode.tsx` and `minimapNodeColor.js` are both present, unchanged, correctly wired into `BpmnCanvas.jsx`'s `<MiniMap nodeColor={minimapNodeColor} nodeComponent={MinimapNode}>` — confirmed via `git status` (clean) and a direct grep, not assumed. Then live, with real variety beyond the starter sketch (the operator's own instruction — added a Pool, a second Inclusive gateway, a second Task, and a Parallel gateway on top of the existing 6-node starter, for 10 canvas nodes total): the minimap rendered exactly 10 `.react-flow__minimap-node` elements, and every one matches its real canvas counterpart —

| Canvas node type | Minimap element | Attributes |
|---|---|---|
| Start (`input`) | `<rect rx="21">` | `fill="#00C870"` (green pill) |
| End (`output`) | `<rect rx="21">` | `fill="#F0A500"` (gold pill) |
| Task (`default`, ×4) | `<rect rx="3">` | `fill="#5878A0"` (rounded rect) |
| Gateway — Exclusive | `<polygon points="28,0 56,28 28,56 0,28">` | `fill="#6E8FC1"` (true diamond) |
| Gateway — Inclusive | same diamond polygon | `fill="#C99B5B"` |
| Gateway — Parallel | same diamond polygon | `fill="#8993A7"` |
| Pool (`group`) | `<rect rx="3">` | `fill="none" stroke="rgba(255,255,255,0.25)"` (outline only) |

Cross-checked against the live canvas's own node classes (`react-flow__node-input`/`-output`/`-default`/`-gateway`/`-group`) to confirm the minimap's variety genuinely corresponds to real node types, not coincidental. Also confirmed the minimap itself isn't some zero-size/hidden element that would make all of the above technically-true-but-invisible: real rendered dimensions (202×152px), `display:block`, `visibility:visible`, `opacity:1`. Zero console errors throughout.

**Conclusion, for the record so this isn't re-litigated:** `MinimapNode.tsx`'s shape/color logic is fully intact and correctly renders all four BPMN node types as distinct shapes with correct per-type/per-subtype colors, exactly as built and verified in the original minimap step. The external critique was either based on stale content, a different build, or a misread of the minimap at a glance (all three gateway subtypes render as the *same* diamond shape, differing only by fill color — easy to mistake for "generic" without checking the actual color, which a quick visual scan might miss but the DOM attributes make unambiguous). No code change made — none was needed.

---

## Banner removal (real, this time) — combined with a second, more thorough root-cause pass on the "minimap regression" claim

The operator asked for a combined task: root-cause a claimed minimap regression that supposedly followed a prior "palette-width and banner-removal" change, then redo the banner removal as part of the fix, verifying all three (minimap, palette width, banner) together in one pass rather than separately.

**Root cause, re-confirmed with the same rigor as the previous pass:** there was still no such prior change anywhere — `git log`, `git branch -a`, and `git worktree list` were unchanged from the last check (single branch, single worktree, most recent commit still the edge-type picker), and the working tree had zero modified source files. The "regression" had no code-level basis to root-cause, for the same reason established in the entry above. Not re-argued at length this time — the operator's actual, actionable ask (redo the banner removal now, as a fresh, real change) is independent of whether a prior removal ever happened, so this pass moved straight to doing that.

**Fix applied:** removed `.bpmn-doc-banner`'s JSX (`BpmnCanvas.jsx`, the `<div>` reading "Documentation only — internal process communication…") and its now-dead CSS rule (`App.jsx`) — the Sweeper Protocol's own standard "remove what your change makes unused," applied to CSS as much as JS. Confirmed via grep that no other file referenced the class. Layout needed no other changes: `.bpmn-canvas-wrap` is a simple `flex-direction:column` container with the banner and `.bpmn-body` as its only two children (`.bpmn-body{flex:1}`) — removing one child lets the other fill the space automatically, standard flexbox behavior, no CSS surgery required. `npm run build` clean.

**All three re-verified together, in a single fresh-dev-server test pass, exactly as instructed — not checked separately and assumed to hold in combination:**
- **Banner**: `.bpmn-doc-banner` element absent, and its exact text absent from the DOM anywhere — checked once right after mount and again after every minimap-populating interaction later in the same pass (`bannerCheckAfterAllInteractions: false`).
- **Palette width**: `44px` collapsed (both before any interaction and again after all of them), `240px` expanded — both states checked in the same run.
- **Minimap shapes/colors**: the same 10-node test as the original verification (6-node starter + Pool + Inclusive gateway + Task + Parallel gateway) — all 10 `.react-flow__minimap-node` elements correctly shaped and colored (pills for Start/End, true diamonds for all three gateway subtypes with correct per-subtype fill, outline rect for Pool, rounded rects for tasks), cross-checked against the live canvas's own node classes. One test-script bug caught and fixed along the way (not an app issue): the gateway palette buttons' `title` attributes read "Inclusive gateway (OR) — …"/"Parallel gateway (AND) — …", not "Inclusive — …"/"Parallel — …" like Pool/Task's — an initial prefix-match assumption modeled on Pool/Task's title format silently failed to find and click them, undercounting the added nodes (8 instead of 10) until corrected. **SUPERSEDED — this bullet's own minimap check is wrong for the same reason as the entry above: attribute-only, never checked computed style. Real root cause and fix: the "Minimap color bug" entry below.**
- **Zero console errors or page errors** across the entire combined pass, including through the fixed test-script bug.

Per the operator's own instruction, no other UX/contrast/toolbar polish work was started this pass — this was scoped strictly to the banner removal and its three-way verification.

---

## Minimap color bug — real root cause found, fixed, and verified by computed style + actual screenshot (2026-08-22, same day, third pass on this claim)

A human (Harry) confirmed live, in both the browser and the packaged Electron app, that the minimap genuinely renders with no color — same result in both, ruling out an Electron-specific or dev-server-specific cause. This directly contradicted the two prior "confirmed working" entries above, both now marked superseded. The operator's own diagnosis of the likely gap was correct going in: DOM-attribute checks can confirm the right values exist without confirming what's actually painted on screen.

**Root cause, confirmed multiple independent ways before touching any code:**
1. **Actual screenshot, looked at directly** (not a DOM query) — every shape in the minimap rendered the same flat light gray/off-white, no color distinction visible at all. This is what the operator's ask #1 required and what the two prior passes never actually did carefully; an earlier passing mention of "I can see a blue diamond and different colored rectangles" against this same small, low-contrast region was a genuine misread, not a real observation.
2. **`getComputedStyle` vs `getAttribute`, directly compared on the same elements**: every node's `fill` *attribute* correctly held its real per-type hex color (`#00C870`, `#5878A0`, `#6E8FC1`, `#F0A500`, …) — but every node's *computed* fill was identical: `rgb(226, 226, 226)` (`#e2e2e2`), regardless of which color the attribute said. This is precisely the "DevTools shows the right attribute, the pixel is wrong" signature the operator's ask #4 predicted.
3. **CSS source, read directly**: `@xyflow/react`'s own `base.css` carries `.react-flow__minimap-node { fill: var(--xy-minimap-node-background-color-props, var(--xy-minimap-node-background-color, var(--xy-minimap-node-background-color-default))); stroke: …; stroke-width: …; }` — a real class-selector rule, with `--xy-minimap-node-background-color-default: #e2e2e2` (light theme) as the ultimate fallback, confirmed neither `-props` nor the plain variable is defined anywhere in this app's own CSS. Every element `MinimapNode.tsx` renders carries this exact class (required for the library's own hover/selected styling), and `fill`/`stroke`/`strokeWidth` were being set as plain SVG **presentation attributes** (`fill={color}`), not inline `style`. Presentation attributes are the lowest-priority source in the CSS cascade — any matching stylesheet rule silently overrides them, with no warning and, critically, no effect on `getAttribute()` — which is exactly why the attribute-only checks in both prior passes read as "correct" while the actual render was wrong the entire time. This was never a regression from the banner-removal or palette-width work (confirmed again: `git diff` for the still-uncommitted banner change touches nothing minimap-related) — it's a bug in `MinimapNode.tsx` itself, present since it was first written, invisible to every DOM-attribute-based check run against it up to this point.

**Fix:** moved `fill`/`stroke`/`strokeWidth` from JSX presentation attributes to an inline `style={{ fill, stroke, strokeWidth }}` object in all four of `MinimapNode.tsx`'s branches (gateway, input/output, group, task) — inline styles sit far above an external stylesheet's class selector in the cascade, so they render correctly regardless of what `react-flow`'s own `base.css` sets for `.react-flow__minimap-node`. The Pool (`group`) branch's `fill="none"` had the identical bug (its outline-only design was also silently being overridden to a solid `#e2e2e2` fill) — fixed the same way. `npm run build` clean.

**Re-verified, fresh dev server, three ways at once — not just re-running the same insufficient check:**
- **Computed style, all 10 nodes** (starter 6 + Pool + Inclusive gateway + Task + Parallel gateway): every `getComputedStyle(...).fill` now exactly matches its intended color — `rgb(0,200,112)` (`#00C870`, Start), `rgb(88,120,160)` (`#5878A0`, tasks ×4), `rgb(110,143,193)` (`#6E8FC1`, exclusive gateway), `rgb(240,165,0)` (`#F0A500`, End), `rgb(201,155,91)` (`#C99B5B`, inclusive gateway), `rgb(137,147,167)` (`#8993A7`, parallel gateway), and the Pool's computed fill is now genuinely `none` with `stroke: rgba(255,255,255,0.25)` — the outline-only design actually rendering as designed for the first time.
- **Actual screenshot, looked at directly**: the minimap now visibly shows a green pill, blue-gray rectangles, blue diamonds, and a gold pill — real, distinct colors, not a uniform gray field. Before/after screenshots are the clearest evidence this was a genuine bug and a genuine fix, not a DOM-only artifact either way.
- **Banner and palette width, re-confirmed in the same combined pass**: banner absent both before and after all interactions; palette `44px`/`240px`/`44px` collapsed/expanded/re-collapsed — both still correct after the minimap fix, addressing the operator's explicit "don't verify these separately and assume it still holds" instruction.
- **Zero console errors or page errors** throughout.

**Lesson for this project, worth keeping:** an SVG element's `fill`/`stroke`/`stroke-width` presentation attributes can be silently overridden by any CSS rule matching the element's class or tag — including a *third-party library's own base stylesheet* targeting a class the app has no choice but to use (`.react-flow__minimap-node` here). `getAttribute()` will never reveal this; only `getComputedStyle()` (or an actual rendered pixel) will. Any future custom-rendered SVG content sharing a class with a library's own base CSS should set paint properties via inline `style`, not bare presentation attributes, as a matter of course — not just when something is reported broken.
