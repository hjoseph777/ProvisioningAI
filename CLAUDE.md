...
**Done (2026-08-22, later same session — scoped UX polish pass, Process Docs canvas only):** palette expanded width narrowed 240px → 200px (`.bpmn-pal-shell.pinned`/`.bpmn-pal-panel`, `src/App.jsx`) after live-measuring real content at seven candidate widths — no tile label ever needed more than 170px, the real constraint was the edge-type picker's hint text wrapping unevenly at the old width; 200px is the narrowest point where all three cards wrap to a uniform 2 lines. Real contrast failures found via `getComputedStyle()` (not CSS source) and fixed: toolbar buttons (`.xb`) measured 3.93:1, the edge-type picker's hint text measured 1.42:1 (functionally illegible) — both against their real live backgrounds. Fixed via one new token, `--mid2:#6E90BE` (an AA-passing `--mid`/`--dim` stand-in, ~5:1 against the real backgrounds involved), applied only to the specific selectors the audit flagged, not a global `--mid`/`--dim` redefinition. Hover-highlight standardized to `--a3` blue across the canvas's toolbar/palette/edge-picker (confirmed via `AskUserQuestion` first) — the canvas already used blue almost everywhere; closed the real gaps (edge-type hint text, `.bpmn-history-item-count`, `.bpmn-status-bar-chevron`, none of which changed color on hover before). Toolbar: added the missing divider/group wrapper between History and Shortcuts, and normalized every icon glyph (mixed emoji + monospace arrows) to an identical 13×11px footprint via a `.bpmn-tb-icon` span, rather than swapping to an SVG icon library. The previous 4 separate Export BPMN/Import BPMN/Save/Load buttons were replaced with 2 unified "Export ▾"/"Import ▾" dropdowns; Export gained two new formats never previously offered, SVG and PNG canvas snapshots, via a new `src/features/export-import/exportImage.js` using `html-to-image` (**new dependency**, confirmed as the standard pure-client-side pattern for `@xyflow/react` 12 — not the deferred server-side-image-creation feature) — underlying JSON/BPMN-XML logic in `saveLoadSlice.js`/`bpmnIoSlice.js` untouched, this was a UI consolidation only. All four export formats verified against real downloaded file bytes (via a real Playwright `download` event, not a mocked one) — SVG/PNG carry genuine image content, not placeholders. Full detail, every measurement, and before/after screenshots: progress.md's four matching 2026-08-22 "UX polish pass" entries.

**Done (2026-08-22, follow-up session):** Animate's traveling dot now colors per branch outcome (green/red) read from the edge's own real `label` text (e.g. the starter sketch's existing `"valid"`/`"invalid"` labels), confirmed via `AskUserQuestion` before implementing since "different colors per path scenario" had at least three genuinely different readings. Edge stroke/routing itself untouched — dot color only. Separately, right-click Undo/Redo was verified live (real right-clicks, real state-change checks) to already work correctly across all three object types — node, edge, and empty-canvas/pane — so the toolbar's standalone Undo/Redo buttons were removed as redundant; Ctrl+Z/Ctrl+Shift+Z reconfirmed as a fully independent third entry point afterward. Full evidence: progress.md's two matching "follow-up session" entries.
...
- **An SVG element's `fill`/`stroke`/`stroke-width` set as a plain JSX/presentation attribute (`fill={color}`) can be silently overridden by any CSS rule matching the element's class or tag — `getAttribute()` will never reveal this, only `getComputedStyle()` (or the actual rendered pixels) will.** Hit `features/minimap/MinimapNode.tsx` for real, found 2026-08-22 after a human reported the minimap rendering with no color in both the browser and the packaged Electron app: `@xyflow/react`'s own `base.css` sets `fill`/`stroke`/`stroke-width` on `.react-flow__minimap-node`, a class every custom minimap node here has to carry — every node's `fill` *attribute* held its correct per-type color, but every node's *computed*, actually-painted fill was the library's own flat default (`#e2e2e2`), because presentation attributes are the lowest-priority source in the CSS cascade and lose to any matching stylesheet rule. Two earlier verification passes both missed this because they only checked `getAttribute('fill')`. Fixed by moving paint properties to inline `style={{fill, stroke, strokeWidth}}` instead, which sits above an external stylesheet's class selector. Full root-cause writeup and before/after evidence (computed styles, real screenshots): progress.md's "Minimap color bug" entry (2026-08-22). Any future custom SVG content sharing a class with a library's own base CSS should set paint via inline `style` from the start, not just once something is reported broken.
